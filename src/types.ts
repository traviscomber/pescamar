...
export type Lot = { id:string; plantId:string; species:Species; supplier:string; initials:string; zone:string; guide:number; gross:number; tare:number; drained:number; accepted:number; loss:number; gonadYield:number|null; premiumYield:number|null; temperature:number; status:LotStatus; receivedAt:string; evidenceCount:number; evidence:ReceptionEvidence[] }
