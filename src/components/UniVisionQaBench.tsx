...
import {analyzeSegmentedCanvas,type UniVisionSegmentation} from '../lib/uniVisionSegmentationV3'
...
export function UniVisionQaBench(){
 ...
 const reviewReasons=result?[result.roi.source==='default'?'No se detectó una cavidad de bandeja con suficiente confianza':null,result.borderCandidateRatio>0.16?'La máscara todavía toca demasiado el perímetro del área útil':null,result.usableRatio<0.02?'Hay muy poco producto aislado':null,result.usableRatio>0.88?'La máscara cubre demasiado del área útil y puede estar incluyendo contenedor':null].filter(Boolean) as string[]:[]
 return <section className="panel" aria-label="Banco QA Uni Vision">
  <div className="section-heading"><div><span className="overline teal">Validación visual asistida · scan v3</span><h2>Revisar producto con una foto</h2></div><ScanLine size={20}/></div>
