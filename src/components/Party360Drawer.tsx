...
export function Party360Drawer({party,metric,canEdit,onClose,onEdit}:{party:Party360Party|null;metric?:Party360Metric;canEdit:boolean;onClose:()=>void;onEdit:()=>void}){
 ...
 return <div className="lot-drawer-backdrop" role="presentation" onMouseDown={event=>{if(event.currentTarget===event.target)onClose()}}>
  <aside className="lot-drawer" role="dialog" aria-modal="true" aria-label={`Ficha 360 de ${party.legal_name}`}>
   <header className="lot-drawer-header"><div><span className="overline teal">Ficha 360 · {party.kind==='supplier'?'Proveedor':'Cliente'}</span><h2>{party.legal_name}</h2><p>{location||'Pescamar'}</p></div><button className="icon-button" onClick={onClose} aria-label="Cerrar ficha"><X size={20}/></button></header>
