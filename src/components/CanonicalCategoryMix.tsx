...
type Site={site:string;rows:number;receivedKg:number;massReviewRows:number;eligibleRows:number;massReconciledPct:number|null;reconciledCategoryKg:number}
type Payload={ok?:boolean;method?:{version:string;rule:string};summary?:{rows:number;eligibleRows:number;excludedRows:number;missingOutputRows:number;reconciledCategoryKg:number};categories?:Category[];suppliers?:Supplier[];sites?:Site[];error?:string}
...
export function CanonicalCategoryMix(){
 ...
 return <section className="panel" aria-label="Mix canónico de categorías reconciliadas" aria-live="polite">
  ...
  {suppliers.length?<div className="table-scroll"><table className="data-table"><thead><tr><th>Proveedor</th><th className="numeric">Filas</th><th className="numeric">Recibido</th><th className="numeric">Masa reconciliada</th><th className="numeric">Fuera por masa</th><th className="numeric">Kg categorías válidas</th></tr></thead><tbody>{suppliers.map(item=><tr key={item.supplier}><td><b>{item.supplier}</b></td><td className="numeric">{item.rows}</td><td className="numeric">{kg(item.receivedKg)}</td><td className={`numeric ${item.massReviewRows?'negative':''}`}>{pct(item.massReconciledPct)}</td><td className={`numeric ${item.massReviewRows?'negative':''}`}>{item.massReviewRows}</td><td className="numeric">{kg(item.reconciledCategoryKg)}</td></tr>)}</tbody></table></div>:null}
  {sites.length?<><div className="section-heading"><div><span className="overline">Patrón de captura</span><h3>Reconciliación por planta de proceso</h3><p className="source-note">Si las exclusiones se concentran por planta, la causa puede ser una convención de registro distinta. Esta señal se usa para investigar la captura antes de atribuir el problema al proveedor.</p></div><span>{sites.length} plantas</span></div><div className="table-scroll"><table className="data-table"><thead><tr><th>Planta / sitio</th><th className="numeric">Filas</th><th className="numeric">Recibido</th><th className="numeric">Masa reconciliada</th><th className="numeric">Fuera por masa</th><th className="numeric">Kg categorías válidas</th></tr></thead><tbody>{sites.map(item=><tr key={item.site}><td><b>{item.site}</b></td><td className="numeric">{item.rows}</td><td className="numeric">{kg(item.receivedKg)}</td><td className={`numeric ${item.massReviewRows?'negative':''}`}>{pct(item.massReconciledPct)}</td><td className={`numeric ${item.massReviewRows?'negative':''}`}>{item.massReviewRows}</td><td className="numeric">{kg(item.reconciledCategoryKg)}</td></tr>)}</tbody></table></div></>:null}
