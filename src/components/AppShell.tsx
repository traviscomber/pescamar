...
export function AppShell({children,onNewReception}:{children:ReactNode;onNewReception:()=>void}){
 ...
 const context=pathname==="/"?"Estado operativo de hoy":pathname.startsWith("/proceso-erizo")?"Proceso de erizo":pathname.startsWith("/floor")?"Piso / packing":pathname.startsWith("/pallets")?"Palletización":pathname.startsWith("/frio")?"Cadena de frío":pathname.startsWith("/control-regulatorio")?"Control regulatorio":pathname.startsWith("/estaciones")?"Estaciones y dispositivos":pathname.startsWith("/impresion-etiquetas")?"Impresión de etiquetas":pathname.startsWith("/etiquetas")?"Etiquetas":pathname.startsWith("/timeline")?"Línea de tiempo":pathname.startsWith("/comunicaciones")?"Comunicaciones":pathname.startsWith("/auditoria")?"Auditoría operacional":pathname.startsWith("/observabilidad")?"Observabilidad y alertas":pathname.startsWith("/planificacion")?"Planificación":pathname.startsWith("/proveedores-clientes")?"Proveedores y clientes":pathname.startsWith("/rentabilidad")?"Rentabilidad":pathname.startsWith("/aprobaciones")?"Decisiones":pathname.startsWith("/recepciones")?"Recepciones":pathname.startsWith("/inventario-materiales")?"Materias primas e insumos":pathname.startsWith("/inventario")?"Inventario":pathname.startsWith("/costos-transformacion")?"Costos":pathname.startsWith("/ordenes-venta")?"Órdenes de venta":pathname.startsWith("/despachos-ventas")?"Despachos y ventas":pathname.startsWith("/identidades-plantas")?"Identidades históricas":pathname.startsWith("/plantas")?"Plantas":pathname.startsWith("/lineas")?"Producción":pathname.startsWith("/creditos")?"Créditos y anticipos":pathname.startsWith("/liquidaciones")?"Liquidaciones":pathname.startsWith("/importaciones")?"Importaciones":pathname.startsWith("/operadores")?"Operadores":"Configuración";
 ...
 const mayCreate=operator?canCreateReception(operator.role):false,platformLabel=!status?"Verificando":status.ok?"Plataforma activa":"Revisar plataforma",databaseLabel=!status?"Sincronizando estado":status.persistence.database?"Base de Datos Pescamar conectada":"Base de Datos Pescamar pendiente";
 return <div className="app-shell"><a className="skip-link" href="#main-content">Ir al contenido principal</a><aside ref={drawerRef} className={`sidebar ${mobileOpen?"is-open":""}`} aria-label="Navegación de Pescamar"><div className="brand"><span className="pescamar-symbol" aria-hidden="true"><svg viewBox="0 0 52 34" role="img"><path d="M4 17c8-8 17-12 27-10 5 1 10 4 15 10-5 6-10 9-15 10-10 2-19-2-27-10Z"/><path d="M36 11c4-4 8-6 12-6-1 5-1 8 0 12-4 0-8-2-12-6Z"/><circle cx="14" cy="15" r="1.35"/><path className="brand-wave" d="M3 27c8-3 15-3 22 0s15 3 24-1"/></svg></span><div className="brand-copy"><strong className="brand-name">Pescamar</strong><small className="brand-product">Control operacional</small></div><button className="icon-btn mobile-close" onClick={()=>setMobileOpen(false)} aria-label="Cerrar menú"><X size={18}/></button></div><div className="plant-chip"><span className="live-dot pending"/><div><b>Red Pescamar</b><small>{operator?.role==="admin"?"Cobertura corporativa":operator?.plantIds.length?`${operator.plantIds.length} planta${operator.plantIds.length===1?"":"s"} bajo tu alcance`:"Sin plantas asignadas"}</small></div></div><nav className="side-nav grouped-nav"><section><small>Trabajo diario</small>{visiblePrimary.map(({to,label,icon:Icon})=><NavLink key={to} to={to} end={to==="/"}><Icon size={18}/><span>{label}</span></NavLink>)}</section>{visibleGroups.map(({label,icon:GroupIcon,items})=><details className="nav-more" key={label} open={items.some(item=>pathname.startsWith(item.to))}><summary><GroupIcon size={18}/><span>{label}</span></summary><div>{items.map(({to,label:itemLabel,icon:Icon})=><NavLink key={to} to={to}><Icon size={18}/><span>{itemLabel}</span></NavLink>)}</div></details>)}</nav><div className="sidebar-spacer"/>{operator&&canAccessPath(operator.role,"/modulos")?<NavLink className="settings-link" to="/modulos"><Settings2 size={18}/><span>Configuración</span></NavLink>:null}</aside>{mobileOpen?<button className="mobile-nav-backdrop" type="button" onClick={()=>setMobileOpen(false)} aria-label="Cerrar navegación"/>:null}<div className="workspace"><header className="topbar"><button ref={menuButtonRef} className="icon-btn menu-btn" onClick={()=>setMobileOpen(true)} aria-label="Abrir menú"><Menu size={20}/></button><div className="topbar-context"><span>Pescamar</span><b>{context}</b></div><div className="topbar-actions"><button className="theme-toggle" type="button" onClick={()=>setTheme(theme==="dark"?"light":"dark")}>{theme==="dark"?<Sun size={15}/>:<Moon size={15}/>}<span>{theme==="dark"?"Claro":"Oscuro"}</span></button>{operator&&canAccessPath(operator.role,"/modulos")?<NavLink className="system-state" to="/modulos"><Activity size={15}/><span><b>{platformLabel}</b><small>{databaseLabel}</small></span></NavLink>:<div className="system-state"><Activity size={15}/><span><b>{platformLabel}</b><small>{databaseLabel}</small></span></div>}<button className="operator-state operator-session-button" type="button" onClick={()=>void logout()}><span>{initials}</span><div><b>{operator?.fullName??"Sesión operativa"}</b><small>{operator?roleLabels[operator.role]:"Operador"}</small></div><LogOut size={14}/></button></div></header><main id="main-content" tabIndex={-1} className="main-content">{children}</main></div>{mayCreate?<button className="floating-action" onClick={onNewReception}>+ Nueva recepción</button>:null}</div>
}
