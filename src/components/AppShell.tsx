import {Activity,Bot,Factory,History,LayoutDashboard,LogOut,Menu,Moon,PackageCheck,Settings2,ShieldCheck,ShoppingCart,Sun,TrendingUp,X} from "lucide-react";
...
import {isCeoOperator,isPrimaryOperatorPath} from "../roleExperience";
...
export function AppShell({children,onNewReception}:{children:ReactNode;onNewReception:()=>void}){
 ...
 const ceoView=isCeoOperator(operator),operationsView=operator?.role==='operations';
 const showFloatingReception=mayCreate&&pathname==="/"&&!operationsView&&!ceoView,sessionLabel=operator?`${operator.fullName} · ${ceoView?'CEO':t(roleKey)}`:t('shell.session');
 ...
 const allowed=(path:string)=>Boolean(operator&&canAccessPath(operator.role,path));
 const primary=(path:string)=>Boolean(operator&&canAccessPath(operator.role,path)&&isPrimaryOperatorPath(operator,path));
 const cleanExecutiveView=operationsView||ceoView;
 return <div className="app-shell"><a className="skip-link" href="#main-content">{t('shell.skip')}</a><aside ref={drawerRef} className={`sidebar ${mobileOpen?"is-open":""}`} aria-label={`${t('shell.navigation')} · Pescamar`}><div className="brand"><span className="pescamar-symbol" aria-hidden="true"><svg viewBox="0 0 52 34" role="img"><path d="M4 17c8-8 17-12 27-10 5 1 10 4 15 10-5 6-10 9-15 10-10 2-19-2-27-10Z"/><path d="M36 11c4-4 8-6 12-6-1 5-1 8 0 12-4 0-8-2-12-6Z"/><circle cx="14" cy="15" r="1.35"/><path className="brand-wave" d="M3 27c8-3 15-3 22 0s15 3 24-1"/></svg></span><div className="brand-copy"><strong className="brand-name">PESCAMAR</strong><small className="brand-product">Seafood Intelligence OS</small></div><button className="icon-btn mobile-close" onClick={()=>setMobileOpen(false)} aria-label={t('shell.closeMenu')}><X size={18}/></button></div><div className="sidebar-section-label">{ceoView?(locale==='es'?'Dirección':'Executive'): 'Trabajo diario'}</div><nav className="side-nav workspace-nav" aria-label={ceoView?(locale==='es'?'Dirección':'Executive'):'Trabajo diario'}>{primary('/')?<NavLink to="/" end><LayoutDashboard size={18}/><span>{t('nav.home')}</span></NavLink>:null}{primary('/pescamar-ia')?<NavLink to="/pescamar-ia"><Bot size={18}/><span>Seafood AI</span></NavLink>:null}{primary('/rentabilidad')?<NavLink to="/rentabilidad"><TrendingUp size={18}/><span>{locale==='es'?'Rentabilidad':'Profitability'}</span></NavLink>:null}{primary('/recepciones')?<NavLink to="/recepciones"><Activity size={18}/><span>{t('nav.reception')}</span></NavLink>:null}{primary('/lineas')?<NavLink to="/lineas"><Factory size={18}/><span>{t('nav.production')}</span></NavLink>:null}{primary('/control-regulatorio')?<NavLink to="/control-regulatorio"><ShieldCheck size={18}/><span>{t('nav.quality')}</span></NavLink>:null}{primary('/inventario')?<NavLink to="/inventario"><PackageCheck size={18}/><span>{t('nav.inventory')}</span></NavLink>:null}{primary('/ordenes-venta')?<NavLink to="/ordenes-venta"><ShoppingCart size={18}/><span>{t('nav.sales')}</span></NavLink>:null}</nav><div className="sidebar-spacer"/>{!cleanExecutiveView?<><div className="sidebar-section-label secondary">Consulta</div><nav className="side-nav sidebar-secondary" aria-label="Consulta y configuración">{allowed('/lineage')?<NavLink to="/lineage"><History size={18}/><span>{t('nav.history')}</span></NavLink>:null}<NavLink to="/inicio/detalle"><Activity size={18}/><span>{t('nav.reports')}</span></NavLink>{allowed('/modulos')?<NavLink to="/modulos" title={t('nav.settings')}><Settings2 size={18}/><span>{t('nav.admin')}</span></NavLink>:null}</nav></>:null}<a className="powered-by-link" href="https://www.n3uralia.com" target="_blank" rel="noreferrer">Powered by N3uralia</a></aside>{mobileOpen?<button className="mobile-nav-backdrop" type="button" onClick={()=>setMobileOpen(false)} aria-label={t('shell.closeNavigation')}/>:null}<div className="workspace"><header className="topbar"><button ref={menuButtonRef} className="icon-btn menu-btn" onClick={()=>setMobileOpen(true)} aria-label={t('shell.openMenu')}><Menu size={20}/></button><div className="topbar-context"><span>{ceoView?'CEO':contextStage}</span><b>{ceoView?(locale==='es'?'Pescamar · Dirección ejecutiva':'Pescamar · Executive'):context}</b></div><div className="topbar-actions"><button className="compact-control" type="button" onClick={switchLocale} title={locale==='es'?'English':'Español'} aria-label={locale==='es'?'Switch to English':'Cambiar a español'}>{locale==='es'?'EN':'ES'}</button><button className="theme-toggle compact-control" type="button" title={themeLabel} aria-label={themeLabel} onClick={()=>setTheme(theme==="dark"?"light":"dark")}>{theme==="dark"?<Sun size={15}/>:<Moon size={15}/>}</button>{operator&&canAccessPath(operator.role,"/modulos")&&!ceoView?<NavLink className={`system-state compact-control ${status?.ok?"is-ok":"is-pending"}`} to="/modulos" title={`${platformLabel} · ${databaseLabel}`} aria-label={`${platformLabel}. ${databaseLabel}`}><Activity size={15}/></NavLink>:!ceoView?<div className={`system-state compact-control ${status?.ok?"is-ok":"is-pending"}`} title={`${platformLabel} · ${databaseLabel}`} aria-label={`${platformLabel}. ${databaseLabel}`}><Activity size={15}/></div>:null}<button className="operator-state operator-session-button compact-operator" type="button" onClick={()=>void logout()} title={`${sessionLabel} · ${t('shell.logout')}`} aria-label={`${t('shell.logout')} · ${sessionLabel}`}><span>{initials}</span><LogOut size={13}/></button></div></header>{tabs.length&&!ceoView?<nav className={`workspace-tabs ${flowClass}`} aria-label={tabsLabel}>{tabs.map(item=><NavLink key={item.to} to={item.to} className={()=>pathname===item.to.split('?')[0]||pathname.startsWith(`${item.to.split('?')[0]}/`)?"active":undefined}>{item.step?<span className="workspace-step-index">{String(item.step).padStart(2,"0")}</span>:null}<span>{t(item.labelKey)}</span></NavLink>)}</nav>:null}<main id="main-content" tabIndex={-1} className="main-content">{children}</main></div>{showFloatingReception?<button className="floating-action" onClick={onNewReception}>{t('shell.newReception')}</button>:null}</div>
}
