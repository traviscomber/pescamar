...
export function CanonicalInventoryEvidence(){
 ...
 const packingProduct=packingSource?.product_family?titleCase(packingSource.product_family):'Packing'
 ...
 return <section className="panel canonical-inventory-evidence" aria-label="Evidencia canónica de inventario">
  <div className="section-heading"><div><span className="overline">Evidencia canónica · solo lectura</span><h2>Stock canónico + {packingProduct.toLowerCase()}</h2></div><span>{data.summary?.packingBoxes??0} cajas · {flags} observaciones</span></div>
  <div className="signal-grid">
   ...
   <article className="signal-card"><span><Boxes size={16}/>{packingProduct}</span><b>{nf.format(Number(data.summary?.packingKg??0))} kg</b><small>{data.summary?.packingBoxes??0} cajas BLOQUE + IQF</small></article>
