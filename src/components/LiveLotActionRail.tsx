...
type LineagePayload={events?:LineageEvent[];coverage?:LineageCoverage;permissions?:{canSeeCommercial?:boolean};error?:string}
...
export function LiveLotActionRail({receptionId}:{receptionId:string|null}){
 ...
 const next=useMemo<NextAction|null>(()=>{
  ...
  const lineageEvents=data?.lineage.events??[]
  const palletEvent=[...lineageEvents].reverse().find(event=>event.type==='pallet')
  ...
  const hasInventoryLocation=coverage.inventory===true&&lineageEvents.some(event=>event.type==='inventory'&&typeof event.metrics?.toLocationId==='string'&&event.metrics.toLocationId.trim().length>0)
  const hasCommercialCommitment=coverage.commercialCommitment===true&&lineageEvents.some(event=>event.type==='commercial_commitment'&&Number(event.metrics?.allocatedKg)>0&&String(event.metrics?.status??'').toLowerCase()!=='cancelled')
  const canSeeCommercial=data?.lineage.permissions?.canSeeCommercial!==false
  ...
  if(!hasInventoryLocation)return{label:'Ubicar producto',detail:'La cadena física está registrada. Confirma dónde queda físicamente este lote antes de comprometerlo.',to:`/inventario?${q}`,icon:'inventory',step:6,completed:5}
  if(!hasCommercialCommitment&&canSeeCommercial)return{label:'Comprometer producto',detail:'El stock ya está ubicado. Reserva este lote sólo contra una orden comercial real.',to:`/ordenes-venta?${q}`,icon:'commercial',step:6,completed:5}
  if(!hasCommercialCommitment&&!canSeeCommercial&&!hasDispatch)return{label:'Compromiso comercial pendiente',detail:'Operaciones, Finanzas o Administración debe confirmar la reserva antes del despacho.',to:`/inventario?${q}`,icon:'inventory',step:6,completed:5}
  if(!hasDispatch)return{label:'Preparar despacho',detail:'La ubicación y el compromiso comercial están respaldados; registra la salida física.',to:`/despachos-ventas?${q}`,icon:'commercial',step:7,completed:6}
