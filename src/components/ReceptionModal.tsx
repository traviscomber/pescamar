...
const sameName=(a:string,b:string)=>clean(a).toLocaleLowerCase('es-CL')===clean(b).toLocaleLowerCase('es-CL');
...
const receptionIdentity=(occurredAt:string,guideReference:string)=>`ER-${new Date(occurredAt).getTime()}-${clean(guideReference).replace(/[^A-Za-z0-9]/g,'').slice(-12)}`;
...
export function ReceptionModal({open,onClose,onSave}:{open:boolean;onClose:()=>void;onSave:(lot:Lot)=>Promise<string>}) {
  ...
  const selectedPlant=accessiblePlants.find((plant)=>plant.id===selectedPlantId)??null;
  const [supplier,setSupplier]=useState(""),[supplierHint,setSupplierHint]=useState(""),[suppliers,setSuppliers]=useState<MasterParty[]>([]),[suppliersLoading,setSuppliersLoading]=useState(false),[supplierLoadError,setSupplierLoadError]=useState("");
  const [zone,setZone]=useState(""),[guideReference,setGuideReference]=useState("");
  ...
  const formRef=useRef<HTMLFormElement|null>(null),previousFocus=useRef<HTMLElement|null>(null),savingRef=useRef(false),grossRef=useRef<HTMLInputElement|null>(null);
  ...
  const documentaryReady=Boolean(supplier&&guideReference.trim()&&zone.trim()&&guideValue!=null);

  useEffect(()=>{savingRef.current=saving},[saving]);
  useEffect(()=>{if(!open)return;setPlantId("");setSupplier("");setSupplierHint("");setZone("");setGuideReference("");setSpecies("Erizo");setOccurredAt(localDateTime());setGuide("");setGross("");setTare("");setDrained("");setTemperature("");setEvidence([]);setEvidenceKind("document");setEvidenceLabel("");setEvidenceUrl("");setEvidenceNote("");setError("");setSupplierLoadError("");setSaving(false)},[open]);
  useEffect(()=>{if(!open)return;let active=true;setSuppliersLoading(true);setSupplierLoadError('');void fetch('/api/partners',{cache:'no-store'}).then(async response=>{const payload=await response.json() as PartnersPayload;if(!response.ok)throw new Error(payload.error??'No fue posible cargar el maestro de proveedores');if(active)setSuppliers(payload.parties??[])}).catch(cause=>{if(active){setSuppliers([]);setSupplierLoadError(cause instanceof Error?cause.message:'No fue posible cargar el maestro de proveedores')}}).finally(()=>{if(active)setSuppliersLoading(false)});return()=>{active=false}},[open]);
  useEffect(()=>{if(!open)return;previousFocus.current=document.activeElement instanceof HTMLElement?document.activeElement:null;const previousOverflow=document.body.style.overflow;document.body.style.overflow="hidden";const frame=window.requestAnimationFrame(()=>grossRef.current?.focus());const onKeyDown=(event:KeyboardEvent)=>{if(event.key==="Escape"&&!savingRef.current){event.preventDefault();onClose();return}if(event.key!=="Tab")return;const focusable=Array.from(formRef.current?.querySelectorAll<HTMLElement>('button:not([disabled]),a[href],input:not([disabled]),select:not([disabled]),textarea:not([disabled]),summary,[tabindex]:not([tabindex="-1"])')??[]).filter((element)=>element.offsetParent!==null);if(!focusable.length)return;const first=focusable[0],last=focusable[focusable.length-1];if(event.shiftKey&&document.activeElement===first){event.preventDefault();last.focus()}else if(!event.shiftKey&&document.activeElement===last){event.preventDefault();first.focus()}};document.addEventListener("keydown",onKeyDown);return()=>{window.cancelAnimationFrame(frame);document.removeEventListener("keydown",onKeyDown);document.body.style.overflow=previousOverflow;const target=previousFocus.current;if(target&&document.contains(target))target.focus()}},[open,onClose]);

  if(!open)return null;
  function selectMasterSupplier(candidate:string){const cleaned=clean(candidate),match=activeSuppliers.find((party)=>sameName(party.legal_name,cleaned));if(match){setSupplier(match.legal_name);setSupplierHint('');setError('');return true}setSupplierHint(cleaned);setError('El proveedor sugerido no está activo en el maestro. Selecciona una ficha maestra o complétala en Proveedores y clientes.');return false}
  ...
  function applyVision(fields:{supplier:string|null;guideReference:string|null;zone:string|null;species:string|null;guide:number|null;gross:number|null;tare:number|null;drained:number|null;temperature:number|null;occurredAt:string|null}){
    if(fields.supplier)selectMasterSupplier(fields.supplier);
  ...
  function validate(){if(!selectedPlantId)return "Tu identidad no tiene una planta habilitada";if(!activeSuppliers.some((party)=>party.legal_name===supplier))return "Confirma el proveedor documental";if(clean(zone).length<2)return "Confirma la zona de extracción";if(clean(guideReference).length<2)return "Confirma el folio o referencia de la guía";const occurred=new Date(occurredAt);if(Number.isNaN(occurred.getTime()))return "La fecha/hora de recepción no es válida";if(guideValue==null||grossValue==null||tareValue==null||drainedValue==null||temperatureValue==null)return "Confirma el contexto documental y las cuatro mediciones físicas";if(guideValue<=0||grossValue<=0)return "Los pesos de guía y bruto deben ser mayores a cero";if(tareValue<0||tareValue>grossValue)return "La tara no puede superar el peso bruto";if(drainedValue<0||drainedValue>grossValue)return "El peso escurrido debe estar entre 0 y el peso bruto";if(tareValue>drainedValue)return "La tara no puede superar el peso escurrido";if(temperatureValue< -5||temperatureValue>30)return "La temperatura está fuera del rango operativo admitido (-5 a 30 °C)";return ""}
  async function submit(event:FormEvent){event.preventDefault();setError("");const issue=validate();if(issue){setError(issue);return}const finalGuide=guideValue!,finalGross=grossValue!,finalTare=tareValue!,finalDrained=drainedValue!,finalTemperature=temperatureValue!,finalAccepted=accepted!,finalLoss=loss??0;setSaving(true);const normalizedSupplier=clean(supplier),initials=normalizedSupplier.split(" ").map((x)=>x[0]).slice(0,2).join("");const id=receptionIdentity(occurredAt,guideReference);try{const receptionId=await onSave({id,plantId:selectedPlantId,species,supplier:normalizedSupplier,initials,zone:clean(zone),guide:finalGuide,guideReference:clean(guideReference),gross:finalGross,tare:finalTare,drained:finalDrained,accepted:finalAccepted,loss:finalLoss,gonadYield:null,premiumYield:null,temperature:finalTemperature,status:"Muestreo",receivedAt:new Date(occurredAt).toLocaleTimeString("es-CL",{hour:"2-digit",minute:"2-digit"}),occurredAt:new Date(occurredAt).toISOString(),evidenceCount:evidence.length,evidence});onClose();openLive(receptionId)}catch(cause){setError(cause instanceof Error?cause.message:"No fue posible crear la recepción")}finally{setSaving(false)}}
  ...
  return <div className="modal-backdrop" onMouseDown={(event)=>{if(event.target===event.currentTarget&&!saving)onClose()}}><form ref={formRef} className="modal-panel" onSubmit={submit} role="dialog" aria-modal="true" aria-labelledby="reception-modal-title">
    <header><div><span className="overline teal">Inicio de operación viva</span><h2 id="reception-modal-title">Nueva recepción</h2><p>Sube la evidencia, confirma sólo las diferencias y registra las mediciones físicas.</p></div><button type="button" className="icon-btn" onClick={onClose} aria-label="Cerrar" disabled={saving}><X size={19}/></button></header>
    ...
    <section className="reception-evidence-panel"><header><div><span className="overline teal">1 · Evidencia y contexto</span><h3>Leer guía y preparar recepción</h3></div><span>{evidence.length}/6</span></header>
      ...
      <div className={`reception-context-summary ${documentaryReady?'ready':'pending'}`} aria-live="polite">
        <div><small>Planta</small><b>{selectedPlant?.name??'Sin planta habilitada'}</b></div>
        <div><small>Hora</small><b>{new Date(occurredAt).toLocaleString('es-CL',{hour:'2-digit',minute:'2-digit'})}</b></div>
        <div><small>Proveedor</small><b>{supplier||'Pendiente de evidencia'}</b></div>
        <div><small>Guía</small><b>{guideReference||'Pendiente'}</b></div>
        <div><small>Zona / especie</small><b>{zone?`${zone} · ${species}`:species}</b></div>
        <div><small>Peso guía</small><b>{displayKg(guideValue)}</b></div>
      </div>
      <details className="manual-evidence context-correction"><summary>{documentaryReady?'Corregir contexto documental':'Completar contexto manualmente'}</summary><div className="form-grid reception-context-grid">
        {accessiblePlants.length>1?<label>Planta<select value={selectedPlantId} onChange={(event)=>setPlantId(event.target.value)} required>{accessiblePlants.map((plant)=><option value={plant.id} key={plant.id}>{plant.name}</option>)}</select></label>:null}
        <label>Fecha y hora<input type="datetime-local" value={occurredAt} max={localDateTime()} onChange={(event)=>setOccurredAt(event.target.value)} required/></label>
        <label>Proveedor<select required value={supplier} onChange={(event)=>{setSupplier(event.target.value);setSupplierHint('');setError('')}} disabled={suppliersLoading||!activeSuppliers.length}><option value="">{suppliersLoading?'Cargando maestro…':activeSuppliers.length?'Selecciona un proveedor':'Sin proveedores activos'}</option>{activeSuppliers.map((party)=><option value={party.legal_name} key={party.id}>{party.legal_name}</option>)}</select>{supplierHint?<small>Vision sugirió “{supplierHint}”, pero no coincide con una ficha activa.</small>:null}{!suppliersLoading&&!activeSuppliers.length?<small>No hay proveedores activos. Completa una ficha en <a href="/proveedores-clientes">Proveedores y clientes</a>.</small>:null}</label>
      ...
      {supplierLoadError?<p className="form-error" role="alert">{supplierLoadError}</p>:null}
      <SupplierReceptionDecision supplier={supplier} zone={zone} species={species} onUseSupplier={selectMasterSupplier}/>
      <details className="manual-evidence"><summary>Agregar enlace de evidencia manual</summary><div className="evidence-input-grid">
      ...
      {evidence.length?<div className="evidence-list">{evidence.map((item,index)=><article key={`${item.url}-${index}`}><FileCheck2 size={17}/><div><b>{item.label}</b><small>{item.note||item.url}</small></div><button type="button" className="icon-btn" aria-label={`Quitar ${item.label}`} onClick={()=>setEvidence((current)=>current.filter((_,itemIndex)=>itemIndex!==index))}><Trash2 size={15}/></button></article>)}</div>:<p className="evidence-empty">Vision puede leer proveedor, guía, zona, especie y peso declarado. Las mediciones físicas nunca se aceptan desde Vision.</p>}
    </section>

    <section className="reception-physical-panel" aria-labelledby="reception-physical-title"><header><div><span className="overline teal">2 · Confirmación física</span><h3 id="reception-physical-title">Cuatro mediciones del operador</h3><p>Estos valores provienen de la recepción física y no se completan automáticamente.</p></div></header><div className="form-grid reception-physical-grid">
      <label>Peso bruto (kg)<div className="input-icon"><Scale size={16}/><input ref={grossRef} type="number" inputMode="decimal" min="0.1" step=".1" value={gross} onChange={(event)=>setGross(event.target.value)} placeholder="0,0" required/></div></label>
      ...
      <label>Peso escurrido (kg)<input type="number" inputMode="decimal" min="0" step=".1" value={drained} onChange={(event)=>setDrained(event.target.value)} placeholder="0,0" required/></label>
    ...
    {error?<p className="form-error" role="alert">{error}</p>:null}<footer><button type="button" className="button secondary" onClick={onClose} disabled={saving}>Cancelar</button><button className="button primary" type="submit" disabled={saving||!selectedPlantId||suppliersLoading}>{saving?"Guardando recepción…":"Confirmar recepción y abrir Ficha 360"}</button></footer>
  </form></div>
}