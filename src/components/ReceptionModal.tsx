...
export function ReceptionModal({open,onClose,onSave}:{open:boolean;onClose:()=>void;onSave:(lot:Lot)=>Promise<string>}) {
  ...
  const formRef=useRef<HTMLFormElement|null>(null),closeRef=useRef<HTMLButtonElement|null>(null),supplierRef=useRef<HTMLSelectElement|null>(null),previousFocus=useRef<HTMLElement|null>(null),savingRef=useRef(false);
  ...
  return <div className="modal-backdrop" onMouseDown={(event)=>{if(event.target===event.currentTarget&&!saving)onClose()}}><form ref={formRef} className="modal-panel" onSubmit={submit} role="dialog" aria-modal="true" aria-labelledby="reception-modal-title">
    ...
    <div className="form-grid">
      ...
      <label>Proveedor<select ref={supplierRef} required value={supplier} onChange={(event)=>{setSupplier(event.target.value);setSupplierHint('');setError('')}} disabled={suppliersLoading||!activeSuppliers.length}><option value="">{suppliersLoading?'Cargando maestro…':activeSuppliers.length?'Selecciona un proveedor del maestro':'Sin proveedores activos'}</option>{activeSuppliers.map((party)=><option value={party.legal_name} key={party.id}>{party.legal_name}</option>)}</select>{supplierHint?<small>La evidencia sugirió “{supplierHint}”. Confirma una ficha activa del maestro antes de continuar.</small>:null}{!suppliersLoading&&!activeSuppliers.length?<small>No hay proveedores activos. Completa una ficha en <a href="/proveedores-clientes">Proveedores y clientes</a>.</small>:null}</label>
    ...
    {error?<p className="form-error" role="alert">{error}</p>:null}<footer><button type="button" className="button secondary" onClick={onClose} disabled={saving}>Cancelar</button><button className="button primary" type="submit" disabled={saving||!selectedPlantId||suppliersLoading||!supplier}>{saving?"Guardando recepción…":"Crear recepción y abrir Ficha 360"}</button></footer>
  </form></div>
}