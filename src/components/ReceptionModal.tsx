...
export function ReceptionModal({open,onClose,onSave}:{open:boolean;onClose:()=>void;onSave:(lot:Lot)=>Promise<string>}) {
  ...
  function validate(){if(!selectedPlantId)return "Tu identidad no tiene una planta habilitada";if(!activeSuppliers.some((party)=>party.legal_name===supplier))return "Confirma el proveedor documental";if(clean(zone).length<2)return "Confirma la zona de extracción";if(clean(guideReference).length<2)return "Confirma el folio o referencia de la guía";const occurred=new Date(occurredAt);if(Number.isNaN(occurred.getTime())||occurred.getTime()>Date.now()+15*60*1000)return "La fecha/hora de recepción no es válida";if(guideValue==null||grossValue==null||tareValue==null||drainedValue==null||temperatureValue==null)return "Confirma el contexto documental y las cuatro mediciones físicas";if(guideValue<=0||grossValue<=0)return "Los pesos de guía y bruto deben ser mayores a cero";if(tareValue<0||tareValue>grossValue)return "La tara no puede superar el peso bruto";if(drainedValue<0||drainedValue>grossValue)return "El peso escurrido debe estar entre 0 y el peso bruto";if(tareValue>drainedValue)return "La tara no puede superar el peso escurrido";if(temperatureValue< -5||temperatureValue>30)return "La temperatura está fuera del rango operativo admitido (-5 a 30 °C)";return ""}
