...
export function OperationalHealthPanel({expanded=false}:{expanded?:boolean}){
 ...
 const StateIcon=statusIcon(data.status),shownAlerts=expanded?alerts:alerts.filter(item=>item.severity!=='info').slice(0,3),shownChecks=expanded?(data.checks??[]):(data.checks??[]).filter(item=>item.status!=='healthy').slice(0,4)
 ...
 return <section className={`operational-health ${expanded?'expanded':''}`} aria-label="Observabilidad operacional" aria-live="polite">
  <div className={`operational-health-hero ${data.status}`}><div className="operational-health-title"><StateIcon size={20}/><div><span className="overline">Observabilidad</span><h2>{statusLabel[data.status]} · {data.summary?.critical??0} críticas · {data.summary?.warnings??0} warning</h2><p>{data.status==='healthy'?'Los subsistemas observados no presentan excepciones activas.':'La plataforma sigue operativa, pero hay excepciones concretas que requieren atención.'}</p></div></div><div className="operational-health-meta"><span>Health {data.method?.version??'v1'}</span><b>{data.deployment?.commit??'commit —'}</b><small>{data.checkedAt?new Date(data.checkedAt).toLocaleString('es-CL'):'—'}</small></div></div>
  ...
  {shownAlerts.length?<div className="operational-alert-list">{shownAlerts.map(alert=>{const Icon=domainIcon(alert.domain),allowed=operator&&canAccessPath(operator.role,alert.actionPath);const content=<><span className={`operational-alert-icon ${alert.severity}`}><Icon size={15}/></span><div><small>{alert.domain}</small><b>{alert.title}</b><span>{alert.detail}</span></div>{allowed?<ArrowRight size={15}/>:null}</>;return allowed?<Link key={alert.id} className={`operational-alert ${alert.severity}`} to={alert.actionPath}>{content}</Link>:<div key={alert.id} className={`operational-alert ${alert.severity}`}>{content}</div>})}</div>:<div className="operational-health-clear"><CheckCircle2 size={16}/><span>Sin alertas operacionales activas.</span></div>}
  {shownChecks.length?<div className="operational-check-grid">{shownChecks.map(check=>{const Icon=statusIcon(check.status);return <article key={check.key} className={`operational-check ${check.status}`}><header><Icon size={16}/><div><small>{statusLabel[check.status]}</small><b>{check.label}</b></div></header><p>{check.detail}</p>{expanded&&check.metrics?<dl>{Object.entries(check.metrics).map(([key,value])=><div key={key}><dt>{key}</dt><dd>{metricValue(value)}</dd></div>)}</dl>:null}</article>})}</div>:null}
