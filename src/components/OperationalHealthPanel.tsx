...
export function OperationalHealthPanel({expanded=false}:{expanded?:boolean}){
 ...
 return <section className={`operational-health ${expanded?'expanded':''}`} aria-label="Observabilidad operacional" aria-live="polite">
  <div className={`operational-health-hero ${data.status}`}><div className="operational-health-title"><HealthIcon status={data.status} size={20}/><div><span className="overline">Observabilidad</span><h2>{statusLabel[data.status]} · {data.summary?.critical??0} críticas · {data.summary?.warnings??0} warning</h2><p>{data.status==='healthy'?'Los subsistemas observados no presentan excepciones activas.':'La plataforma sigue operativa, pero hay excepciones concretas que requieren atención.'}</p></div></div><div className="operational-health-meta"><span>Health {data.method?.version??'v1'}</span><b>{data.deployment?.commit??'commit —'}</b><small>{data.checkedAt?new Date(data.checkedAt).toLocaleString('es-CL'):'—'}</small></div></div>
