...
export function SupplierIntelligence(){
 ...
 useEffect(()=>{void load();const refresh=()=>void load(true),onVisibility=()=>{if(document.visibilityState==='visible')refresh()};window.addEventListener('pescamar:data-updated',refresh);window.addEventListener('focus',refresh);document.addEventListener('visibilitychange',onVisibility);return()=>{window.removeEventListener('pescamar:data-updated',refresh);window.removeEventListener('focus',refresh);document.removeEventListener('visibilitychange',onVisibility)}},[load])
 ...
 return <section className="supplier-intelligence" aria-label="Supplier Intelligence" aria-live="polite">
  <div className="supplier-score-hero">
   ...
   <div className="supplier-score-method"><ShieldCheck size={18}/><div><b>{data?.method?.version??'supplier-score-v1'}</b><small>Ciclo live activo: cada control, clasificación y rendimiento nuevo recalcula el score.</small></div></div>
  </div>
...
function SupplierScoreRow({supplier}:{supplier:SupplierScore}){
 const trend=supplier.trendGradeAPoints,live=supplier.liveEvidence
 return <details className="supplier-score-row">
  ...
  <div className="supplier-score-detail">
   ...
   <div className="supplier-score-evidence">
    <div><Scale size={15}/><span><b>{supplier.historicalLots}</b> lotes históricos</span></div><div><Activity size={15}/><span><b>{supplier.liveLots}</b> lotes live</span></div><div><ShieldCheck size={15}/><span><b>{supplier.coverage}%</b> cobertura del score</span></div>{live?<div><TrendingUp size={15}/><span><b>{live.qualitySamples}</b> señales calidad · <b>{live.yieldSamples}</b> rendimiento</span></div>:null}
   </div>
