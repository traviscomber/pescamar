...
export function ExecutiveDecisionBrief(){
 ...
 const ownerForPath=(path:string)=>{
  if(['/ordenes-venta','/proveedores-clientes','/despachos-ventas','/liquidaciones','/creditos','/costos-transformacion'].some(prefix=>path.startsWith(prefix)))return t('brief.ownerCommercial')
  if(['/aprobaciones','/rentabilidad'].some(prefix=>path.startsWith(prefix)))return t('brief.ownerManagement')
  if(['/integrations','/observabilidad','/organization','/operadores','/modulos'].some(prefix=>path.startsWith(prefix)))return t('brief.ownerAdministration')
  return t('home.ownerOperations')
 }
 ...
 if(!scores)return <section className="panel executive-decision-brief loading"><ShieldCheck size={18}/><div><b>{t('brief.checking')}</b><small>{t('brief.checkingDetail')}</small></div></section>
 ...
 const preferredScore=preferred?.purchaseScore==null?t('brief.notEvaluated'):t('brief.scoreDetail',{score:nf.format(preferred.purchaseScore),coverage:nf.format(preferred.supplier.coverage),confidence:preferred.supplier.confidence})
 const operationalOwner=topOperational?ownerForPath(topOperational.path):null
 return <section className="executive-decision-brief" aria-label={t('home.attention')}><div className="section-heading decision-brief-heading"><div><span className="overline teal">{t('brief.eyebrow')}</span><h2>{t('home.attention')}</h2></div></div>
 {topOperational?<Link className={`decision-operational-priority p${topOperational.signal.priority}`} to={topOperational.path}><span className="decision-priority-mark"><ShieldAlert size={19}/><b>{topOperational.signal.priority}</b></span><div><small>{t('brief.priority',{priority:topOperational.signal.priority})}</small><h3>{topOperational.signal.title}</h3><p>{topOperational.signal.detail}</p><span>{topOperational.receptionNumber?t('brief.reception',{number:topOperational.receptionNumber}):t('brief.lot')}{topOperational.supplier?` · ${topOperational.supplier}`:''}</span><strong>{t('brief.do',{action:topOperational.signal.action})}</strong><span><b>{t('home.owner')}:</b> {operationalOwner}</span>{topOperational.signal.blockers.length?<em>{t('brief.blocks',{items:topOperational.signal.blockers.join(' · ')})}</em>:null}</div><div className="decision-priority-counts"><span><b>{operationalCounts.p1}</b>{t('brief.priorityHigh')}</span><span><b>{operationalCounts.p2}</b>{t('brief.priorityMedium')}</span><span><b>{operationalCounts.p3}</b>{t('brief.priorityLow')}</span><ArrowRight size={16}/></div></Link>:<div className="decision-operational-clear"><ShieldCheck size={17}/><div><b>{operational?t('brief.clearTitle',{lots:operational.lots??0}):t('brief.stateUnavailable')}</b><small>{operational?t('brief.clearDetail'):t('brief.noDataDetail')}</small></div>{operational&&operationalTotal===0?<span>{t('brief.clearBadge')}</span>:null}</div>}
 <div className="decision-brief-grid">
  <Link className={`decision-brief-card ${preferred?.decision.level??'neutral'}`} to={supplierPath}><ShoppingCart size={17}/><div><small>{t('brief.purchase')}</small><b>{preferred?.supplier.supplier??t('brief.insufficientInfo')}</b><span>{preferred?`${preferred.decision.label} · ${preferredScore}`:t('brief.comparePending')}</span></div><ArrowRight size={15}/></Link>
  <Link className={`decision-brief-card ${supportTone}`} to={evidencePath}><Layers3 size={17}/><div><small>{t('brief.traceability')}</small><b>{supportReady?t('brief.linked',{linked:support.summary?.autoLinkedBlocks??0,total:support.summary?.blocks??0}):t('brief.pending')}</b><span>{supportReady?t('brief.toReview',{count:support.summary?.exceptions??0}):t('brief.linkPending')}</span></div><ArrowRight size={15}/></Link>
  <Link className={`decision-brief-card ${supplierAttention?.decision.level??'neutral'}`} to={supplierPath}><AlertTriangle size={17}/><div><small>{t('brief.supplier')}</small><b>{supplierAttention?.supplier.supplier??t('brief.noAlert')}</b><span>{supplierAttention?supplierAttention.decision.label:t('brief.noReviewPending')}</span></div><ArrowRight size={15}/></Link>
  <Link className={`decision-brief-card ${weakestPlant&&weakestPlant.score<75?'warning':weakestPlant?'positive':'neutral'}`} to={weakestPlant?`/plantas/${weakestPlant.plantId}`:rolloutPath}><Factory size={17}/><div><small>{t('home.plant')}</small><b>{weakestPlant?`${weakestPlant.plantId} · ${weakestPlant.score}%`:t('brief.notEvaluatedPlant')}</b><span>{weakestPlant?t('brief.checks',{completed:weakestPlant.completed,total:weakestPlant.total}):t('brief.insufficientInfoDetail')}</span></div><ArrowRight size={15}/></Link>
 </div>{supportReady&&(support.summary?.exceptions??0)>0?<div className="decision-brief-exception"><AlertTriangle size={15}/><span><b>{t('brief.quality')}</b> {support.suppliers?.flatMap(item=>item.unresolved).slice(0,2).map(item=>`${item.sheetName} · bloque ${item.sourceBlock} · guía ${item.guide??'—'} · lote ${item.lotReference??'—'}`).join(' | ')}</span></div>:null}</section>
}
