...
export function ExecutiveDecisionBrief(){
 ...
 return <section className="executive-decision-brief" aria-label="Centro de decisión"><div className="section-heading decision-brief-heading"><div><span className="overline teal">Centro de decisión</span><h2>Qué necesita atención ahora</h2></div><small>Una lectura ejecutiva; el detalle sigue en cada módulo.</small></div><div className="decision-brief-grid">
  ...
  <Link className={`decision-brief-card ${weakestPlant&&weakestPlant.score<75?'warning':weakestPlant?'positive':'neutral'}`} to={weakestPlant?`/plantas/${weakestPlant.plantId}`:rolloutPath}><Factory size={17}/><div><small>Readiness planta</small><b>{weakestPlant?`${weakestPlant.plantId} · ${weakestPlant.score}%`:'Sin readiness'}</b><span>{weakestPlant?`${weakestPlant.completed}/${weakestPlant.total} evidencias · ${weakestPlant.metrics.receptions} recepciones vivas`:'Aún no existe evidencia suficiente por planta.'}</span></div><ArrowRight size={15}/></Link>
