...
type Payload={orders?:Order[];inventoryMovements?:Movement[];costs?:Cost[];totalTransformationCostClp?:number;error?:string}
...
const href=(path:string,receptionId:string)=>`${path}?receptionId=${encodeURIComponent(receptionId)}`
...
export function LiveLotContinuityRail({receptionId}:{receptionId:string|null}){
  ...
  const orders=data?.orders??[],moves=data?.inventoryMovements??[],costs=data?.costs??[]
