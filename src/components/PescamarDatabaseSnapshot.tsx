...
export function PescamarDatabaseSnapshot(){
  ...
  return <section className="panel import-history">
    ...
    <div className="signal-grid">
      <Link className="signal-card" to="/lineas?source=pescamar-2026"><span><Scale size={16}/>Producción</span><b>{nf.format(production?.receivedKg??0)} kg</b><small>{production?.receptionPct?.toFixed(1)??'—'}% recibido · {nf.format(production?.rows??0)} registros</small><em>Analizar producción <ArrowRight size={13}/></em></Link>
      ...
      <Link className="signal-card" to="/inventario?source=pescamar-2026"><span><Database size={16}/>Stock</span><b>{nf.format(stockRows)} registros</b><small>Erizo y pulpo preservados como información de stock</small><em>Revisar stock <ArrowRight size={13}/></em></Link>
      {data?.finance?<Link className="signal-card" to="/liquidaciones?source=pescamar-2026"><span><WalletCards size={16}/>Transferencias</span><b>{clp.format(data.finance.transfers.amountClp)}</b><small>{data.finance.transfers.rows} movimientos registrados</small><em>Ir a finanzas <ArrowRight size={13}/></em></Link>:null}
    </div>
