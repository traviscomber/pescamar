...
export function PescamarDatabaseSnapshot(){
  ...
  return <section className="panel import-history">
    ...
    <div className="signal-grid">
      <article className="signal-card"><span><Scale size={16}/>Producción</span><b>{nf.format(production?.receivedKg??0)} kg</b><small>{production?.receptionPct?.toFixed(1)??'—'}% recibido · {nf.format(production?.rows??0)} registros</small></article>
      <article className="signal-card"><span><PackageCheck size={16}/>Packing</span><b>{nf.format(packingKg)} kg</b><small>{nf.format(packingBoxes)} cajas · BLOQUE + IQF</small></article>
      <article className="signal-card"><span><Database size={16}/>Stock</span><b>{nf.format(stockRows)} registros</b><small>Erizo y pulpo preservados como información de stock</small></article>
      {data?.finance?<article className="signal-card"><span><WalletCards size={16}/>Transferencias</span><b>{clp.format(data.finance.transfers.amountClp)}</b><small>{data.finance.transfers.rows} movimientos registrados</small></article>:null}
    </div>
    {data?.finance?<div className="governance-note"><WalletCards size={19}/><div><b>Cuenta corriente</b><p>Entradas {clp.format(data.finance.ledger.inflowClp)} · salidas {clp.format(data.finance.ledger.outflowClp)} · diferencia recalculada {clp.format(data.finance.ledger.balanceClp)}. Se muestra como información de la planilla, no como saldo bancario.</p></div></div>:null}
    {exceptions.length?<div className="alert-list">{exceptions.slice(0,4).map((item,index)=><div className="alert-row static" key={`${item.kind}-${index}`}><span className={`alert-icon ${item.severity}`}><AlertTriangle size={16}/></span><div><b>{item.title}</b><small>{item.detail}</small></div><span/></div>)}</div>:null}
    <p className="data-caveat">Esta información proviene de la Base de Datos Pescamar y permanece separada de recepciones y movimientos vivos hasta que exista una reconciliación verificable.</p>
  </section>
}
