...
export function Receptions({lots,onNew}:{lots:Lot[];onNew:()=>void}){
  ...
  return <>
    <PageHeader eyebrow="Recepciones" title="Recepciones" description={requestedPlantId?'Operación viva filtrada por planta. La base histórica se mantiene separada hasta confirmar equivalencias de sitio.':'La operación viva convive con la historia canónica de Pescamar. Cada fila histórica conserva archivo, hash y fila de origen en su Ficha 360.'} actions={<>{requestedPlantId?<Link className="button secondary" to={`/plantas/${encodeURIComponent(requestedPlantId)}`}>Volver a planta</Link>:null}{mayCreate?<button className="button primary" onClick={onNew}>+ Nueva recepción</button>:null}</>}/>
    <DataContinuityBanner compact/>
    <section className="summary-strip reception-context" aria-label="Base histórica Pescamar">
      <div><small>Base histórica Pescamar</small><b>{Number(summary?.total??0).toLocaleString('es-CL')} registros</b></div>
    ...
    <section className="panel list-panel receptions-workspace">
      <div className="toolbar"><div className="search-box"><Search size={17}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Buscar lote, proveedor, guía, zona o archivo…" aria-label="Buscar recepciones"/></div></div>
      ...
      <div className="panel-header canonical-table-heading"><div><span className="overline">Base canónica</span><h2>Recepciones históricas</h2></div><span>{historicalFiltered.length} de {Number(summary?.total??history.length)}</span></div>
