...
export function Receptions({lots,onNew}:{lots:Lot[];onNew:()=>void}){
  ...
  return <>
    ...
    <section className="panel list-panel receptions-workspace" aria-label="Recepciones activas">
      {scopedLots.length?<>
        <div className="panel-header"><div><span className="overline teal">Ahora</span><h2>{filtered.length} recepción{filtered.length===1?'':'es'} activa{filtered.length===1?'':'s'}</h2><p>Abre un lote para revisar estado, siguiente acción, kilos y documentos asociados.</p></div></div>
    ...
    <details className="panel list-panel receptions-history">
      <summary><span><b>Historial de recepciones</b><small>{historyCount.toLocaleString('es-CL')} registros disponibles · solo consulta</small></span><span>Ver historial</span></summary>
      <div className="receptions-history-body">
        <DataContinuityBanner compact/>
        <section className="summary-strip reception-context" aria-label="Historial de Pescamar">
        ...
        <div className="toolbar"><div className="search-box"><Search size={17}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Buscar lote, proveedor, guía, zona o archivo…" aria-label="Buscar historial"/></div></div>
        {requestedPlantId?<p className="data-caveat">El historial aún no se filtra por esta planta hasta confirmar qué nombres históricos corresponden a cada planta actual.</p>:null}
        <div className="table-scroll mobile-card-scroll"><table className="data-table canonical-receptions mobile-card-table"><thead><tr><th>Fecha</th><th>Lote</th><th>Proveedor</th><th>Guía</th><th className="numeric">Kg recibidos</th><th>Estado</th><th></th></tr></thead><tbody>{historicalFiltered.map(item=>{const flags=item.data_quality_flags?.length??0;return <tr key={item.id} className={item.record_status==='void'?'muted':''}><td data-label="Fecha">{date(item.event_date)}</td><td data-label="Lote"><button className="lot-link" onClick={()=>openRecord(item)}>{item.lot_code}</button></td><td data-label="Proveedor">{item.supplier_name??item.supplier_original??'—'}</td><td data-label="Guía">{item.guide_number??'—'}</td><td data-label="Kg recibidos" className="numeric">{kg(item.received_kg)}</td><td data-label="Estado">{item.record_status==='void'?<span className="status-pill pending">Anulado</span>:flags?<span className="status-pill pending">{flags} obs.</span>:<span className="status-pill active">Revisado</span>}</td><td data-label="Acción"><button className="icon-button" onClick={()=>openRecord(item)} aria-label={`Abrir ficha ${item.lot_code}`}><ChevronRight size={17}/></button></td></tr>})}</tbody></table></div>
      </div>
    </details>
  </>
}