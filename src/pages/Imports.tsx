...
type CanonicalConnection={target:string;mode:string;total?:number|string;reception_ready?:number|string;timing_ready?:number|string;quality_ready?:number|string;review_required?:number|string;duplicate_context?:number|string;missing_received_kg?:number|string;missing_reception_date?:number|string;missing_process_date?:number|string;missing_production_date?:number|string;missing_grade_breakdown?:number|string;suppliers?:number|string;exact?:number|string;missing?:number|string;ambiguous?:number|string;unmatched?:number|string;lots?:number|string;exact_lots?:number|string;unmatched_lots?:number|string;outside_coverage_lots?:number|string;unresolved_within_coverage_lots?:number|string;boxes?:number|string;lot_referenced_boxes?:number|string;unreferenced_boxes?:number|string;unreferenced_kg?:number|string;upstream_last_date?:string|null;packing_first_date?:string|null;packing_last_date?:string|null;product_family?:string|null;kg?:number|string;transfers?:number|string;direct_exact_transfers?:number|string;grouped_exact_transfers?:number|string;grouped_exact_groups?:number|string;matched_transfers?:number|string;rows?:number|string;flagged?:number|string}
...
export function Imports(){
  ...
  const connectionRows=useMemo(()=>{
    ...
    if(!value)return []
    const productionConnection=value.production,productionReview=Number(productionConnection?.review_required??0),productionFlags=Number(productionConnection?.flagged??0),productionDuplicates=Number(productionConnection?.duplicate_context??0)
    ...
    return [
      {key:'production',title:'Producción → operación',detail:`${nf.format(Number(productionConnection?.reception_ready??0))} filas elegibles para recepción · ${nf.format(productionFlags)} con flags · ${nf.format(productionDuplicates)} contextos duplicados · ${nf.format(productionReview)} requieren revisión`,target:productionConnection?.target,status:productionReview>0?'REVISAR':'EXACTO'},
