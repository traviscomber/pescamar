...
export function PalletsFocus(){
 ...
 return <>
  <PageHeader eyebrow="Operación de planta" title="Pallets" description="Consolida cajas, completa la composición y cierra cada pallet cuando esté listo."/>
  ...
  {!loading&&!error?<>
   <section className={`daily-cockpit ${priority?'has-attention':'is-clear'}`}>
    <div className="daily-cockpit-copy"><span className="overline">Estado</span><h2>{priority?'Hay un pallet por completar':'Palletización en orden'}</h2><p>{priority?`${priority.pallet_code} sigue abierto${priority.destination?` para ${priority.destination}`:''}. Completa su composición y ciérralo cuando esté listo.`:'No hay pallets abiertos que requieran intervención.'}</p><div className="daily-cockpit-actions"><Link className="button primary" to="/pallets/detalle">{priority?'Continuar pallet':'Abrir pallets'}<ArrowRight size={15}/></Link></div></div>
