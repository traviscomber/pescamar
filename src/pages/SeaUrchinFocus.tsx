...
type CalibrationSummary={captures:number;confirmed:number;accepted:number;lots:number;agreement:{matched:number;total:number;ratio:number}|null;references:Array<{plant_id:string;grade:string;count:number|string}>;yield?:{lots:number;inputKg:number;outputKg:number;pct:number|null};economics?:null|{traceableLots:number;revenueClp:number;contributionClp:number;contributionPct:number|null}}
...
export function SeaUrchinFocus(){
 ...
 const yieldPct=calibration?.yield?.pct,economics=calibration?.economics
 return <>
