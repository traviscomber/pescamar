import {AlertTriangle,Building2,CheckCircle2,Link2,MessageCircleMore,RefreshCw,Sparkles,Users} from 'lucide-react'
...
type EvidenceLink={entityType:string;entityId:string;lotCode:string|null;guideNumber:string|null}
type Message={id:string;channel_name:string|null;channel_category:string|null;plant_id:string|null;sender_name:string|null;direction:'incoming'|'outgoing';message_type:string;text_body:string|null;media:unknown[];occurred_at:string;insight_id:string|null;insight_category:string|null;severity:string|null;summary:string|null;proposed_action:string|null;confidence:number|string|null;insight_status:string|null;links?:EvidenceLink[]}
...
export function Communications(){
  ...
  return <>
    <PageHeader eyebrow="WhatsApp Intelligence" title="Comunicaciones" description="Red de personas, contrapartes y grupos operacionales. El mensaje RAW se conserva; sólo referencias exactas y únicas de lote o guía se vinculan automáticamente a evidencia canónica." actions={<button className="button" onClick={()=>void load()}><RefreshCw size={15}/>Actualizar</button>}/>
    ...
    <section className="signal-grid communication-overview">
      ...
      <article className="signal-card"><span><Link2 size={16}/>Vínculo canónico</span><b>{linkedCount}</b><small>Mensajes con lote/guía exacto y único</small></article>
    </section>
