...
export function ProductionFocus({lots}:{lots:Lot[]}){
  ...
  return <>
    <PageHeader eyebrow={text('Operación','Operations')} title={text('Producción','Production')} description={text('La producción anterior ya está cargada. Los lotes nuevos se registran desde hoy y se mantienen separados del historial.','Previous production is already loaded. New lots are recorded from today onward and remain separate from historical records.')}/>
    ...
    <section className="panel" aria-label={text('Siguiente acción de producción','Next production action')}>
      ...
      </>:<div className="empty-state"><Factory size={28}/><h3>{hasLots?text('No hay una orden que requiera producción ahora','No order requires production now'):text('Sin lotes nuevos todavía','No new lots yet')}</h3><p>{hasLots?text('No hay una acción productiva pendiente.','There is no pending production action.'):text('La producción histórica sigue disponible abajo; la primera recepción nueva continuará la operación desde hoy.','Historical production remains available below; the first new reception will continue operations from today.')}</p>{!hasLots?<Link className="button primary" to="/recepciones">{text('Ir a recepciones','Go to receptions')}</Link>:null}</div>}
    </section>
