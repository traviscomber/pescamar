  ...
  const primary=totals.unlocated>0
    ...
    :firstBlocked
      ...
      :totals.planning>0
        ...
        :{tone:'neutral',icon:<PackageCheck size={20}/>,eyebrow:text('SIN STOCK ACTUAL','NO CURRENT STOCK'),title:receptionId?text('Este lote aún no tiene stock disponible','This lot does not have available stock yet'):text('Sin movimientos nuevos todavía','No new movements yet'),text:receptionId?text('La Ficha 360 mantendrá este mismo lote cuando exista disponibilidad física.','The 360 record will keep this same lot when physical availability exists.'):text('El stock histórico sigue disponible para consulta. El stock actual comienza con los movimientos nuevos.','Historical stock remains available for reference. Current stock starts with new movements.'),label:text('Ir a Operación','Go to Operations'),to:'/recepciones'}
  const guidanceAction=totals.unlocated>0?text('Registrar ubicación','Record location'):firstBlocked?text('Resolver la retención antes de prometer o despachar','Resolve the hold before promising or dispatching'):totals.planning>0?text('Usar sólo los kilos disponibles para planificación','Use only kilograms available for planning'):text('No asumir stock live desde el histórico','Do not infer live stock from history')
  const guidanceReason=totals.unlocated>0?text('Sin ubicación no existe continuidad física suficiente para saber dónde está el producto.','Without a location there is not enough physical continuity to know where the product is.'):firstBlocked?text('Un lote retenido no debe convertirse en disponibilidad comercial hasta resolver sus razones de bloqueo.','A held lot must not become commercial availability until its blockers are resolved.'):totals.planning>0?text('Los kilos mostrados ya descuentan restricciones conocidas; la disponibilidad histórica permanece sólo como referencia.','The kilograms shown already discount known restrictions; historical availability remains reference only.'):text('Ausencia de movimientos actuales no significa ausencia histórica: son dos estados distintos y se mantienen separados.','No current movements does not mean no historical stock: these are distinct states and remain separate.')
  const assistantPrompt=totals.unlocated>0
    ...
    :firstBlocked
      ...
      :totals.planning>0
        ...
        :text('Explícame por qué no hay stock live disponible y separa claramente esa conclusión del inventario histórico de referencia.','Explain why there is no live stock available and clearly separate that conclusion from historical reference inventory.')
  const assistantReceptionId=receptionId||firstBlocked?.reception_id||null
  return <>
    <PageHeader eyebrow={text('Operación de planta','Plant operations')} title={text('Inventario','Inventory')} description={receptionId?text('Disponibilidad y próxima acción del lote activo, sin mezclarlo con otros lotes de la planta.','Availability and next action for the active lot, without mixing it with other plant lots.'):text('Producto disponible hoy y stock histórico, claramente separados.','Product available today and historical stock, clearly separated.')}/>
