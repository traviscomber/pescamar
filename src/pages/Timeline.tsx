...
async function fetchTimeline(){
  ...
  return {...base,events:[...(base.events??[]),...(extra.events??[])].sort((a,b)=>Date.parse(b.occurredAt)-Date.parse(a.occurredAt))}
}
...
export function Timeline(){
  ...
  return <>
    ...
    {loading&&!events.length?<div className="panel timeline-loading"><LoaderCircle className="spin"/><span>Cargando continuidad…</span></div>:<section className="infinite-timeline"><div className="timeline-spine" aria-hidden="true"/>{visible.map((event,index)=>{const Icon=icons[event.module]??Database;const previous=visible[index-1];const year=new Date(event.occurredAt).getFullYear();const previousYear=previous?new Date(previous.occurredAt).getFullYear():null;const historicalNavigable=event.historical&&Number.isFinite(Number(event.metrics?.sourceRow));const liveNavigable=typeof event.metrics?.receptionId==='string'&&event.metrics.receptionId.length>0;const navigable=historicalNavigable||liveNavigable;return <div key={event.id}>{year!==previousYear?<div className="timeline-year"><span>{year}</span><small>{event.historical?'Base canónica':'Operación viva'}</small></div>:null}<article className={`timeline-event ${event.historical?'historical':''} ${event.severity??'normal'} ${navigable?'navigable':''}`} onClick={navigable?()=>openEvent(event):undefined} onKeyDown={navigable?key=>{if(key.key==='Enter'||key.key===' '){key.preventDefault();openEvent(event)}}:undefined} role={navigable?'button':undefined} tabIndex={navigable?0:undefined} aria-label={navigable?`Abrir ficha 360 de ${String(event.metrics?.lotCode??event.title)}`:undefined}><span className="timeline-node"><Icon size={15}/></span><div className="timeline-event-main"><div className="timeline-event-meta"><span>{moduleLabels[event.module]??event.module}</span>{event.plantId?<span>{plantLabels[event.plantId]??event.plantId}</span>:null}<em>{event.historical?'Canónico 2025':'Operación viva'}</em></div><h3>{event.title}</h3><p>{event.detail}</p>{event.actor?<small>{event.actor}</small>:null}</div><time dateTime={event.occurredAt}>{fmt(event.occurredAt)}</time></article></div>})}<div className="timeline-origin"><ArrowDown size={16}/><div><b>Origen canónico</b><small>394 filas de 2025: 393 operacionales y un registro anulado, conservados sin fabricar datos faltantes.</small></div></div></section>}
  </>
}
