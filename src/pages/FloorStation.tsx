...
export function FloorStation(){
 ...
 return <>
  ...
  <section className="floor-status-strip" aria-label="Estado de estación">
   ...
   <div><ScanLine size={18}/><span><b>Scanner</b><small>Adapter pendiente</small></span></div>
  ...
  <section className="floor-console" aria-label="Consola de operación">
   <div className="floor-controls">
    <label>Planta<select value={effectivePlant} onChange={event=>{setPlantId(event.target.value);setLotId("")}} disabled={!plantIds.length}>{plantIds.map(id=><option key={id} value={id}>{id}</option>)}</select></label>
    <label>Lote<select value={selected?.receptionId??""} onChange={event=>setLotId(event.target.value)} disabled={!plantLots.length}>{plantLots.map(lot=><option key={lot.receptionId} value={lot.receptionId}>{lot.id} · {lot.species} · {lot.supplier}</option>)}</select></label>
   </div>
