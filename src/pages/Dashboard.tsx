...
import {PescamarDatabaseSnapshot} from "../components/PescamarDatabaseSnapshot";
...
export function Dashboard({lots,onNewReception}:{lots:Lot[];onNewReception:()=>void}){
  ...
  const {openLive,summary,loading:historyLoading,error:historyError}=useLot360();
  ...
  const baseCount=Number(summary?.total??0);
  const historyReady=!historyLoading&&!historyError&&baseCount>0;
  const ready=historyReady&&Boolean(status?.persistence.database)&&Boolean(operator&&(operator.role==='admin'||operator.plantIds.length>0));
  ...
  return <>
    <PageHeader eyebrow="Hoy" title="Operación" description={liveStarted?"Lo importante, en orden de acción.":"La Base de Datos Pescamar conserva la historia y la operación viva comienza con la primera recepción real."} actions={headerActions}/>

    <PescamarDatabaseSnapshot/>

    {!liveStarted?<section className="operational-start" aria-labelledby="operational-start-title"><div className="operational-start-copy"><span className="overline">Listo para operar</span><h2 id="operational-start-title">La continuidad empieza con una recepción real.</h2><p>La Base de Datos Pescamar permanece intacta. Desde la primera recepción nueva, el mismo lote continúa por Calidad, Producción, Inventario, Costos, Despacho y Venta.</p><div className="operational-start-actions">{mayCreate?<button className="button primary" onClick={onNewReception}><Plus size={15}/>Registrar primera recepción</button>:null}<Link className="button secondary" to="/timeline"><History size={15}/>Ver historial</Link></div></div><div className="readiness-list"><Readiness ready={historyReady} label="Base de Datos Pescamar" detail={historyLoading?'Verificando historial':historyError?'Historial no disponible':baseCount>0?`${baseCount} registros históricos`:'Sin registros históricos'}/><Readiness ready={Boolean(status?.persistence.database)} label="Base operacional" detail={status?.persistence.database?'Conectada':'Revisar conexión'}/><Readiness ready={ready} current={ready} label="Siguiente paso" detail={ready?'Registrar primera recepción':'Completar preparación'}/></div></section>:loading&&!overview?<section className="panel" aria-live="polite"><div className="empty-inline"><RefreshCw size={20}/><div><b>Sincronizando operación</b><small>Cargando el estado vivo antes de mostrar prioridades.</small></div></div></section>:overviewError&&!overview?<section className="panel" role="alert"><div className="notice error"><AlertTriangle size={15}/><span>{overviewError}</span><button className="button secondary" type="button" onClick={()=>void loadOverview(false)}>Reintentar</button></div></section>:<>
    ...
    <section className="secondary-action"><div><span className="overline">Continuidad</span><p>La Base de Datos Pescamar conserva el historial. Todo dato nuevo continúa la misma historia operacional.</p></div><Link className="source-link compact" to="/timeline"><History size={14}/>Línea de tiempo</Link></section>
  </>;
}
