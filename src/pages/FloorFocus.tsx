...
export function FloorFocus(){
 ...
 useEffect(()=>{let live=true;(async()=>{try{const r=await fetch('/api/plant-execution',{cache:'no-store'}),p=await r.json() as Exec;if(!r.ok)throw new Error(p.error??'No fue posible verificar la estación');if(!live)return;setWrites(p.writesEnabled===true);if(p.writesEnabled===true){const sr=await fetch('/api/plant-stations',{cache:'no-store'}),sp=await sr.json() as Stations;if(!sr.ok)throw new Error(sp.error??'No fue posible cargar estaciones');if(live)setStations(sp.stations??[])}}catch(e){if(live){setWrites(false);setError(e instanceof Error?e.message:'No fue posible verificar la estación')}}})();return()=>{live=false}},[])
 ...
 return <>
  ...
  <section className="panel decision-focus">
   ...
   <div className="signal-grid compact-signals"><article className="signal-card"><span>Lotes disponibles</span><b>{allowedLots.length}</b></article><article className="signal-card"><span>Estaciones activas</span><b>{floorStations.length}</b></article><article className="signal-card"><span>Registro</span><b>{writes?'Habilitado':'Detenido'}</b></article></div>
   <div className="page-actions"><Link className="button primary" to="/floor/detalle">{ready?'Operar estación':'Resolver configuración'}<ArrowRight size={15}/></Link><Link className="button" to="/estaciones">Ver estaciones</Link></div>
