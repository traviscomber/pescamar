...
export function DailyClose(){
 ...
 const priorities:Priority[]=[...operationalItems.map(item=>({key:`event-${item.receptionId}-${item.signal.kind}-${item.signal.evidenceEventIds.join('-')}`,score:eventScore(item.signal.priority),reference:`${item.receptionNumber?`REC-${item.receptionNumber}`:'Lote actual'}${item.supplier?` · ${item.supplier}`:''}`,reason:item.signal.title,why:item.signal.detail,next:item.signal.action,to:item.path,action:'Abrir lote',owner:suggestedOwner(item.path),source:'event_graph' as const})),...dailyDistinct.map((item,index)=>{const to=target(item);return {key:`daily-${item.kind}-${item.reference}-${index}`,score:dailyScore(item.level),reference:item.reference,reason:item.reason,why:item.detail,next:action(item),to,action:action(item),owner:suggestedOwner(to),source:'daily' as const}})].sort((a,b)=>b.score-a.score).slice(0,3)
 ...
 return <><PageHeader eyebrow="Pescamar · Hoy" title="Hoy" description="Operación de hoy e historial de Pescamar, claramente separados." actions={actions}/>
 ...
 {snapshot?<>
  <section className={`daily-cockpit ${attention?'has-attention':'is-clear'}`} aria-label="Estado de hoy">
   <div className="daily-cockpit-copy">
    <span className="overline">Operación de hoy</span>
    <h2>{attention?`${attention} asunto${attention===1?'':'s'} para revisar`:'Sin pendientes operacionales'}</h2>
    <p>{attention?'Los pendientes de calidad, producción y comercial aparecen juntos con su siguiente acción y responsable.':'No hay movimientos nuevos pendientes. El historial cargado sigue disponible para consulta debajo.'}</p>
    <div className="daily-cockpit-actions">
     {firstPriority?<Link className="button primary" to={firstPriority.to}>{firstPriority.action}<ArrowRight size={15}/></Link>:<Link className="button primary" to="/recepciones">Registrar nueva recepción<ArrowRight size={15}/></Link>}
     <button className="text-action" type="button" onClick={()=>setShowSummary(value=>!value)}>{showSummary?'Ocultar resumen':'Ver resumen del día'}</button>
    </div>
   </div>
   <div className="daily-status-mark" aria-hidden="true"><span>{attention||'✓'}</span><small>{attention?'por revisar':'sin pendientes'}</small></div>
  </section>

  {priorities.length?<section className="daily-priority" aria-label="Prioridades operacionales"><div className="daily-priority-head"><div><span className="overline">Prioridad 1 de {priorities.length}</span><h2>{priorities[0].reference}</h2><p>{priorities[0].reason}</p></div><Link className="text-action inline-link" to={priorities[0].to}>Abrir<ArrowRight size={14}/></Link></div><small className="daily-priority-detail"><b>Motivo:</b> {priorities[0].why}</small><small className="daily-priority-detail"><b>Siguiente:</b> {priorities[0].next}</small><small className="daily-priority-detail"><b>Responsable:</b> {priorities[0].owner}</small>{priorities[0].source==='event_graph'?<small className="daily-priority-detail"><b>Origen:</b> registros del lote y operación actual</small>:null}{priorities.length>1?<div className="queue-list daily-more-list">{priorities.slice(1).map((item,index)=><Link className="queue-row" to={item.to} key={item.key}><span className="queue-priority">{index+2}</span><div><b>{item.reference}</b><small>{item.reason}</small><small>Siguiente: {item.next}</small><small>Responsable: {item.owner}</small></div><strong>{item.action}</strong><ArrowRight size={15}/></Link>)}</div>:null}</section>:<section className="daily-clear-note"><ShieldCheck size={19}/><div><b>No hay asuntos que requieran intervención</b><small>El historial de Pescamar permanece disponible para consulta y comparación.</small></div></section>}

  {!attention?<section className="daily-clear-context" aria-label="Resumen de la operación actual"><div><small>Lotes actuales</small><b>{operational?.lots??0} lotes</b><span>revisados por el sistema</span></div><div><small>Movimientos del día</small><b>{snapshot.receptions.count+snapshot.production.events+snapshot.dispatches.count}</b><span>{snapshot.receptions.count} recepciones · {snapshot.production.events} producción · {snapshot.dispatches.count} despachos</span></div><div><small>Inventario ubicado</small><b>{kg(snapshot.inventory.locatedKg)}</b><span>producto con ubicación registrada</span></div></section>:null}

  <HistoricalContinuity context="operation"/>

  {showSummary?<section className="daily-reveal" aria-label="Resumen del día"><div className="signal-grid daily-signal-grid"><article className="signal-card"><span><Factory size={16}/>Recibido</span><b>{kg(snapshot.receptions.kg)}</b><small>{snapshot.receptions.count} recepciones</small></article><article className="signal-card"><span><Factory size={16}/>Producido</span><b>{kg(snapshot.production.outputKg)}</b><small>{snapshot.production.events} registros</small></article><article className="signal-card"><span><PackageCheck size={16}/>Inventario</span><b>{kg(snapshot.inventory.locatedKg)}</b><small>ubicado</small></article><article className="signal-card"><span><Truck size={16}/>Despachado</span><b>{kg(snapshot.dispatches.kg)}</b><small>{snapshot.dispatches.count} salidas</small></article></div>{operational?<p className="lot360-caveat">El sistema revisó {operational.lots??0} lote{operational.lots===1?'':'s'} actual{operational.lots===1?'':'es'} y encontró {eventGraphOpen} asunto{eventGraphOpen===1?'':'s'} pendiente{eventGraphOpen===1?'':'s'}.</p>:null}</section>:null}

  <nav className="daily-footer-actions" aria-label="Más información"><Link className="text-action inline-link" to="/pescamar-ia">Consultar análisis<ArrowRight size={14}/></Link><button className="text-action" type="button" onClick={()=>setShowDetail(value=>!value)}>{showDetail?'Cerrar detalle':'Ver detalle operativo'}</button></nav>
