...
type Priority={key:string;score:number;reference:string;reason:string;why:string;next:string;to:string;action:string;source:'event_graph'|'daily'}
 ...
 {snapshot?<>
  <section className={`daily-cockpit ${attention?'has-attention':'is-clear'}`} aria-label="Control Tower de hoy">
   <div className="daily-cockpit-copy">
    ...
    <p>{attention?'Seafood Event Graph y las excepciones del día comparten una sola cola. Las señales P1/P2/P3 mandan; no existe un ranking paralelo de lotes.':'No hay señales P1/P2/P3 ni pendientes visibles dentro del alcance actual. El estado verde conserva contexto operacional para seguir trabajando.'}</p>
