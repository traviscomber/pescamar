...
export function DailyClose(){
 ...
 return <><PageHeader eyebrow="Pescamar · Hoy" title="Hoy" description="Historia canónica + operación live, separadas por evidencia." actions={actions}/>
 ...
 {snapshot?<>
  <section className={`daily-cockpit ${attention?'has-attention':'is-clear'}`} aria-label="Control Tower de hoy">
   <div className="daily-cockpit-copy">
    <span className="overline">Control Tower · Live</span>
    <h2>{attention?`${attention} señal${attention===1?'':'es'} para revisar`:'Sin actividad live pendiente'}</h2>
    <p>{attention?'Seafood Event Graph y las excepciones del día comparten una sola cola. Cada señal indica acción y responsable sugerido.':'La capa live está lista para continuar la operación nueva. La historia cargada permanece disponible abajo como base canónica auditable.'}</p>
    <div className="daily-cockpit-actions">
     {firstPriority?<Link className="button primary" to={firstPriority.to}>{firstPriority.action}<ArrowRight size={15}/></Link>:<Link className="button primary" to="/recepciones">Registrar primera operación live<ArrowRight size={15}/></Link>}
     <button className="text-action" type="button" onClick={()=>setShowSummary(value=>!value)}>{showSummary?'Ocultar resumen':'Ver resumen live'}</button>
    </div>
   </div>
   <div className="daily-status-mark" aria-hidden="true"><span>{attention||'✓'}</span><small>{attention?'por revisar':'live listo'}</small></div>
  </section>

  {priorities.length?<section className="daily-priority" aria-label="Prioridades operacionales"><div className="daily-priority-head"><div><span className="overline">Prioridad 1 de {priorities.length}</span><h2>{priorities[0].reference}</h2><p>{priorities[0].reason}</p></div><Link className="text-action inline-link" to={priorities[0].to}>Abrir<ArrowRight size={14}/></Link></div><small className="daily-priority-detail"><b>Por qué:</b> {priorities[0].why}</small><small className="daily-priority-detail"><b>Siguiente:</b> {priorities[0].next}</small><small className="daily-priority-detail"><b>Responsable sugerido:</b> {priorities[0].owner}</small>{priorities[0].source==='event_graph'?<small className="daily-priority-detail"><b>Fuente:</b> Operational Intelligence · Seafood Event Graph live</small>:null}{priorities.length>1?<div className="queue-list daily-more-list">{priorities.slice(1).map((item,index)=><Link className="queue-row" to={item.to} key={item.key}><span className="queue-priority">{index+2}</span><div><b>{item.reference}</b><small>{item.reason}</small><small>Siguiente: {item.next}</small><small>Responsable sugerido: {item.owner}</small></div><strong>{item.action}</strong><ArrowRight size={15}/></Link>)}</div>:null}</section>:<section className="daily-clear-note"><ShieldCheck size={19}/><div><b>No hay señales live que requieran intervención</b><small>Esto no implica ausencia de historia: la base canónica cargada se mantiene separada y disponible para consulta e inteligencia.</small></div></section>}

  {!attention?<section className="daily-clear-context" aria-label="Contexto operacional live"><div><small>Cobertura live</small><b>{operational?.lots??0} lotes</b><span>revisados por Event Graph</span></div><div><small>Actividad live del día</small><b>{snapshot.receptions.count+snapshot.production.events+snapshot.dispatches.count}</b><span>{snapshot.receptions.count} recepciones · {snapshot.production.events} producción · {snapshot.dispatches.count} despachos</span></div><div><small>Inventario live ubicado</small><b>{kg(snapshot.inventory.locatedKg)}</b><span>evidencia física disponible</span></div></section>:null}

  <HistoricalContinuity context="operation"/>

  {showSummary?<section className="daily-reveal" aria-label="Resumen live del día"><div className="signal-grid daily-signal-grid"><article className="signal-card"><span><Factory size={16}/>Recibido live</span><b>{kg(snapshot.receptions.kg)}</b><small>{snapshot.receptions.count} recepciones</small></article><article className="signal-card"><span><Factory size={16}/>Producido live</span><b>{kg(snapshot.production.outputKg)}</b><small>{snapshot.production.events} eventos</small></article><article className="signal-card"><span><PackageCheck size={16}/>Inventario live</span><b>{kg(snapshot.inventory.locatedKg)}</b><small>ubicado</small></article><article className="signal-card"><span><Truck size={16}/>Despachado live</span><b>{kg(snapshot.dispatches.kg)}</b><small>{snapshot.dispatches.count} salidas</small></article></div>{operational?<p className="lot360-caveat">Operational Intelligence revisó {operational.lots??0} lote{operational.lots===1?'':'s'} live · P1 {operationalCounts.p1} · P2 {operationalCounts.p2} · P3 {operationalCounts.p3} · sólo lectura.</p>:null}</section>:null}
