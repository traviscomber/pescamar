...
export function Copilot(){
 ...
 const ceoPrompts=isEs?['Resumen ejecutivo de hoy','¿Qué requiere mi decisión?','¿Dónde está el mayor riesgo?']:["Today's executive summary",'What requires my decision?','Where is the greatest risk?']
 ...
 const examples=ceoMode?ceoPrompts:operationsMode?operationsPrompts:generalPrompts
 ...
 const operationsValueCopy=isEs?'Cruza recepción, producción, calidad, inventario, pedidos y trazabilidad para darte sólo lo que cambia una decisión.':'Crosses reception, production, quality, inventory, orders and traceability to surface only what changes a decision.'
 const ceoValueCopy=isEs?'Cruza operación, comercial, rentabilidad, histórico y riesgo para mostrar sólo lo que requiere una decisión ejecutiva.':'Crosses operations, commercial, profitability, history and risk to show only what requires an executive decision.'
 const valueCopy=ceoMode?ceoValueCopy:operationsValueCopy
 const description=ceoMode?(isEs?'Seafood AI sintetiza Pescamar completa en resultados conocidos, cambios, riesgos y decisiones que requieren tu intervención.':'Seafood AI summarizes all of Pescamar into known outcomes, changes, risks and decisions requiring your intervention.'):operationsMode?(isEs?'Trabaja como siempre. Seafood AI reúne más datos, detecta excepciones y te ayuda a decidir qué mover ahora.':'Work as usual. Seafood AI brings more data together, detects exceptions and helps you decide what to move now.'):(isEs?'Pregunta por la operación y el historial. El sistema carga sólo la información necesaria y la IA no cambia datos.':'Ask about operations and history. The system loads only the required information, and AI never changes records.')
 return <>
  <PageHeader eyebrow={ceoMode?(isEs?'Dirección ejecutiva':'Executive intelligence'):(isEs?'Inteligencia operacional':'Operational intelligence')} title="Seafood AI" description={description} actions={<><label className="inline-field">{isEs?'Planta':'Plant'}<select value={plantId} onChange={event=>{setPlantId(event.target.value);setReceptionId('');setTurns([])}}><option value="">{operator?.role==='admin'?(isEs?'Toda Pescamar':'All Pescamar'):(isEs?'Mis plantas':'My plants')}</option>{available.map(plant=><option value={plant.id} key={plant.id}>{plant.name}</option>)}</select></label>{!ceoMode?<label className="inline-field">{isEs?'Lote':'Lot'}<select value={receptionId} onChange={event=>{setReceptionId(event.target.value);setTurns([])}}><option value="">{isEs?'Todos':'All'}</option>{selectableLots.map(lot=><option value={lot.receptionId} key={lot.receptionId}>{lot.id} · {lot.species} · {lot.supplier}</option>)}</select></label>:null}<button className="button secondary" type="button" onClick={reset}><RefreshCw size={15}/>{isEs?'Nueva':'New'}</button></>}/>

  {ceoMode?<section className="copilot-context executive" aria-label={isEs?'Contexto ejecutivo':'Executive context'}><div><LockKeyhole size={16}/><span><small>{isEs?'Ámbito':'Scope'}</small><b>{plantId?plantLabel:(isEs?'Pescamar completa':'All Pescamar')}</b></span></div><div><Database size={16}/><span><small>{isEs?'Lectura':'View'}</small><b>{isEs?'Resultados · riesgo · decisiones':'Outcomes · risk · decisions'}</b></span></div><div><ShieldCheck size={16}/><span><small>{isEs?'Control':'Control'}</small><b>{isEs?'Sólo lectura':'Read only'}</b></span></div></section>:<section className={`copilot-context ${executiveMode?'executive':''}`} aria-label={isEs?'Contexto activo':'Active context'}>
  ...
  {executiveMode&&!turns.length&&!selectedLot?<section className="copilot-value"><Sparkles size={18}/><div><b>{ceoMode?(isEs?'Decisiones, no ruido':'Decisions, not noise'):(isEs?'Más contexto, mismo trabajo':'More context, same job')}</b><p>{valueCopy}</p></div></section>:null}
  ...
  <section className={`copilot-workspace panel ${executiveMode?'executive':''}`}>
   <div className="copilot-thread" aria-live="polite">
    {turns.length?turns.map(turn=><article className="copilot-turn" key={turn.id}>
     ...
     <h2>{selectedLot?(isEs?`¿Qué necesitas resolver de ${selectedLot.id}?`:`What do you need to resolve for ${selectedLot.id}?`):ceoMode?(isEs?'¿Qué necesitas decidir hoy?':'What do you need to decide today?'):operationsMode?(isEs?'¿Qué necesitas resolver ahora?':'What do you need to resolve now?'):(isEs?'Pregunta a Pescamar':'Ask Pescamar')}</h2>
   ...
   <form className="copilot-composer" onSubmit={submit}>
    ...
    <small>{ceoMode?(isEs?'Seafood AI sintetiza evidencia autorizada; René mantiene la decisión ejecutiva.':'Seafood AI synthesizes authorized evidence; René keeps the executive decision.'):executiveMode?(isEs?'Seafood AI cruza datos autorizados y recomienda; tú mantienes la decisión.':'Seafood AI combines authorized data and recommends; you keep the decision.'):(isEs?'Seafood AI consulta información autorizada; no modifica registros ni ejecuta decisiones.':'Seafood AI reads authorized information; it does not modify records or execute decisions.')}</small>
   </form>
  </section>
 </>
}
