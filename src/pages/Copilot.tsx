...
function Answer({text,sources,isEs}:{text:string;sources:Source[];isEs:boolean}){
 ...
 const parts=text.split(/(\[(?:receptions|production|quality|inventory|orders|canonical_sources|canonical_inventory|historical_lineage|canonical_intelligence|finance|lot_control|operational_intelligence|urchin_graph|photo_observation|data_readiness)\])/g)
...
export function Copilot(){
 ...
 const generalPrompts=isEs?['¿Qué requiere atención?','¿Qué sabemos del historial?','¿Dónde falta trazabilidad?','¿Qué no sabemos todavía?']:['What requires attention?','What do we know from history?','Where is traceability missing?','What do we still not know?']
