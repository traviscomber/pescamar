...
export type UniVisionSegmentation={metrics:UniVisionMetrics;usableRatio:number;borderCandidateRatio:number;confidence:SegmentationConfidence;maskMode:UniVisionMaskMode;previewDataUrl:string;retainedComponents:number;suppressedFramePixels:number;roi:UniVisionRoi;segmentationVersion:'v2'}
...
function isBroadRoeCandidate([l,a,b]:Lab){const chroma=Math.sqrt(a*a+b*b);return l>16&&l<98&&a>-5&&b>8&&chroma>15}
function isFocusedRoeCandidate([l,a,b]:Lab){const chroma=Math.sqrt(a*a+b*b);return l>36&&l<94&&a>5&&b>24&&chroma>29}
function isStrongRoeSeed([l,a,b]:Lab){const chroma=Math.sqrt(a*a+b*b);return l>34&&l<91&&a>9&&b>29&&chroma>34}
function isTrayLike([l,a,b]:Lab){const chroma=Math.sqrt(a*a+b*b);return l>50&&l<96&&a>-3&&a<19&&b>10&&b<45&&chroma>12&&chroma<49}
...
function detectTrayRoi(full:ImageData,canvas:HTMLCanvasElement):UniVisionRoi{
 ...
 const topCandidates=horizontalScores.map((score,index)=>({score,index})).filter(item=>item.index<rows*0.44&&item.score>=0.34).sort((a,b)=>b.score-a.score)
 const bottomCandidates=horizontalScores.map((score,index)=>({score,index})).filter(item=>item.index>rows*0.56&&item.score>=0.34).sort((a,b)=>b.score-a.score)
 const leftCandidates=verticalScores.map((score,index)=>({score,index})).filter(item=>item.index<cols*0.44&&item.score>=0.24).sort((a,b)=>b.score-a.score)
 const rightCandidates=verticalScores.map((score,index)=>({score,index})).filter(item=>item.index>cols*0.56&&item.score>=0.24).sort((a,b)=>b.score-a.score)
 if(topCandidates.length&&bottomCandidates.length&&leftCandidates.length&&rightCandidates.length){
  ...
  const padX=Math.max(4,Math.round(spanX*0.07)),padY=Math.max(4,Math.round(spanY*0.10))
  const x=Math.max(0,left+padX),y=Math.max(0,top+padY),width=Math.min(canvas.width-x,Math.max(1,spanX-padX*2)),height=Math.min(canvas.height-y,Math.max(1,spanY-padY*2))
  if(width>canvas.width*0.28&&height>canvas.height*0.18)return{x,y,width,height,source:'tray'}
 }
...
function chooseCandidate(full:ImageData,canvas:HTMLCanvasElement,roi:UniVisionRoi,stride:number):Selection{
 ...
 for(let y=roi.y;y<roi.y+roi.height;y+=stride){for(let x=roi.x;x<roi.x+roi.width;x+=stride){
  const index=(y*canvas.width+x)*4;if(full.data[index+3]<200)continue;total++
  const lab=rgbToLab(full.data[index],full.data[index+1],full.data[index+2])
  if(isStrongRoeSeed(lab)){focused++;l+=lab[0];a+=lab[1];b+=lab[2]}
 }}
 const focusedRatio=total?focused/total:0
 if(focused>=80&&focusedRatio>=0.01){
  const centre:[number,number,number]=[l/focused,a/focused,b/focused]
  const adaptive:Candidate=(lab)=>isBroadRoeCandidate(lab)&&(isFocusedRoeCandidate(lab)||deltaE76(lab,centre)<=31)
  return{candidate:adaptive,seed:isStrongRoeSeed,maskMode:'focused'}
 }
...
function suppressLineGeometry(input:Uint8Array,cols:number,rows:number){
 const out=new Uint8Array(input);let suppressed=0
 const clear=(i:number)=>{if(out[i]){out[i]=0;suppressed++}}
 const rowOcc=(y:number)=>{let c=0;for(let x=0;x<cols;x++)c+=input[y*cols+x];return c/cols}
 const colOcc=(x:number)=>{let c=0;for(let y=0;y<rows;y++)c+=input[y*cols+x];return c/rows}
 for(let y=0;y<rows;y++){
  const flags=Array.from({length:cols},(_,x)=>input[y*cols+x]===1),run=longestRun(flags),occ=rowOcc(y),prev=y>0?rowOcc(y-1):occ,next=y<rows-1?rowOcc(y+1):occ
  const edgeZone=y<rows*0.20||y>rows*0.80
  const isolatedRail=run.length>=cols*0.28&&occ>Math.max(prev,next)*1.28&&occ>0.22
  if((edgeZone&&run.length>=cols*0.25)||isolatedRail){for(let x=Math.max(0,run.start);x<=run.end;x++)clear(y*cols+x)}
 }
 for(let x=0;x<cols;x++){
  const flags=Array.from({length:rows},(_,y)=>input[y*cols+x]===1),run=longestRun(flags),occ=colOcc(x),prev=x>0?colOcc(x-1):occ,next=x<cols-1?colOcc(x+1):occ
  const edgeZone=x<cols*0.18||x>cols*0.82
  const isolatedRail=run.length>=rows*0.28&&occ>Math.max(prev,next)*1.28&&occ>0.22
  if((edgeZone&&run.length>=rows*0.25)||isolatedRail){for(let y=Math.max(0,run.start);y<=run.end;y++)clear(y*cols+x)}
 }
...
function spatialMask(full:ImageData,canvas:HTMLCanvasElement,selection:Selection,roi:UniVisionRoi,stride:number):GridMask{
 ...
 const smoothed=smoothMask(raw,cols,rows),lineSuppression=suppressLineGeometry(smoothed,cols,rows),mask=lineSuppression.mask,seen=new Uint8Array(mask.length),out=new Uint8Array(mask.length),minimumArea=Math.max(10,Math.round(mask.length*0.00030))
 let retainedComponents=0;const dirs=[-1,0,1]
 for(let start=0;start<mask.length;start++){
  ...
  const tiny=area<minimumArea,frameLike=bboxArea>mask.length*0.08&&fill<0.14,lineLike=aspect>9&&fill<0.50,edgeArtifact=boundaryRatio>0.20&&seedRatio<0.42,weakColour=seedCount===0&&area<minimumArea*6
  if(tiny||frameLike||lineLike||edgeArtifact||weakColour)continue
  retainedComponents++;for(const i of pixels)out[i]=1
 }
 const closed=smoothMask(out,cols,rows)
 return{cols,rows,stride,originX:roi.x,originY:roi.y,mask:closed,retainedComponents,suppressedFramePixels:lineSuppression.suppressed}
}
...
export function analyzeSegmentedCanvas(canvas:HTMLCanvasElement):UniVisionSegmentation{
 ...
 if(count<100||usableRatio<0.02||grid.retainedComponents===0)throw new Error('No se pudo aislar suficiente roe. Acerca la muestra, reduce reflejos o deja el producto sobre un fondo neutro.')
