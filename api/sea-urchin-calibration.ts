...
export default async function handler(req:Request,res:Response){
 ...
 try{
  ...
  const suppliers=new Map<string,{supplier:string;lots:Set<string>;captures:number;confirmed:number;accepted:number;grades:Record<string,number>;yieldLots:Set<string>;inputKg:number;outputKg:number}>()
  const yieldLots=new Map<string,{receptionId:string;receptionNumber:string;runId:string;supplier:string;inputKg:number;outputKg:number;yieldPct:number}>()
  for(const item of data){
   ...
   const key=String(row.supplier??'Proveedor sin nombre'),supplier=suppliers.get(key)??{supplier:key,lots:new Set<string>(),captures:0,confirmed:0,accepted:0,grades:{A:0,B:0,C:0,D:0,E:0},yieldLots:new Set<string>(),inputKg:0,outputKg:0}
   const receptionId=String(row.reception_id??'')
   supplier.captures++;supplier.lots.add(receptionId);if(row.confirmed_at)supplier.confirmed++;if(row.decision==='accepted')supplier.accepted++;if(['A','B','C','D','E'].includes(confirmedGrade))supplier.grades[confirmedGrade]=(supplier.grades[confirmedGrade]??0)+1
   const inputKg=finite(row.input_kg),outputKg=finite(row.output_kg)
   if(receptionId&&!yieldLots.has(receptionId)&&inputKg!=null&&inputKg>0&&outputKg!=null&&outputKg>0){const yieldPct=outputKg/inputKg*100;yieldLots.set(receptionId,{receptionId,receptionNumber:String(row.reception_number??''),runId:String(row.run_id??''),supplier:key,inputKg,outputKg,yieldPct});supplier.yieldLots.add(receptionId);supplier.inputKg+=inputKg;supplier.outputKg+=outputKg}
