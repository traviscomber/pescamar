...
export default async function handler(req:Request,res:Response){
 ...
 try{
  ...
  const yieldLots=new Map<string,{receptionId:string;receptionNumber:string;runId:string;supplier:string;grade:string|null;inputKg:number;outputKg:number;yieldPct:number;economics:null|{purchaseCostClp:number;transformationCostClp:number;soldKg:number;revenueClp:number;allocatedCostClp:number;contributionClp:number;contributionPct:number|null}}>()
  for(const item of data){
   ...
   if(receptionId&&!yieldLots.has(receptionId)&&inputKg!=null&&inputKg>0&&outputKg!=null&&outputKg>0){
    ...
    let economics:null|{purchaseCostClp:number;transformationCostClp:number;soldKg:number;revenueClp:number;allocatedCostClp:number;contributionClp:number;contributionPct:number|null}=null
