...
function integrity(expected:number,actual:number):CanonicalSourceIntegrity{if(expected===actual)return'complete';if(actual===0)return'empty';return actual<expected?'partial':'over'}
...
export async function buildCanonicalSourceHealth(){
  ...
  return{
    schemaVersion:'seafood.canonical.source-health.v1' as const,
