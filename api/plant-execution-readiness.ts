...
type Row={plant_id:string;stations:string|number;floor_stations:string|number;packing_stations:string|number;cold_stations:string|number;devices:string|number;scanners:string|number;scales:string|number;printers:string|number;sensors:string|number;packing_units:string|number;pallets:string|number;closed_pallets:string|number;linked_cold_assets:string|number;cold_runs_with_evidence:string|number;resolved_holds:string|number}
...
export default async function handler(req:Request,res:Response){
 ...
 try{
  ...
  const rows=await sql`with scoped as (select unnest(${plantIds}::text[]) plant_id),
   ...
   cold_evidence as (select r.plant_id,count(*) cold_runs_with_evidence from cold_runs r where r.plant_id=any(${plantIds}::text[]) and r.observation_count>0 and exists(select 1 from cold_run_loads l where l.run_id=r.id) group by r.plant_id),
  ...
  const plants=((Array.isArray(rows)?rows:[]) as Row[]).map(row=>{
   ...
   const checks=[
    ...
    {key:'cold',label:'Frío con evidencia',complete:metrics.linkedColdAssets>0&&metrics.coldRunsWithEvidence>0,detail:metrics.linkedColdAssets?`${metrics.linkedColdAssets} activo(s) vinculados · ${metrics.coldRunsWithEvidence} ciclo(s) con carga y temperatura`:'Sin activo de frío vinculado a estación'},
