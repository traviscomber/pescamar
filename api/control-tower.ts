...
export default async function handler(req:Request,res:Response){
 ...
 try{
  ...
  const ids=await accessibleReceptionIds(operator,plantId)
  ...
  return res.status(200).json({ok:true,schemaVersion:'control.tower.v2',scope:{plantId,role:operator.role},summary:{live:cards.length,closed,attention,pending,clear:Math.max(0,cards.length-attention-pending),evaluated:ids.length,failed:ids.length-evaluatedCards.length,limit:MAX_EVALUATED},items,generatedAt:new Date().toISOString()})
 }catch(error){
