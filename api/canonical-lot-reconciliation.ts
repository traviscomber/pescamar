...
type LotRow=Record<string,unknown>&{lot_key?:string;lot_labels?:unknown;guides?:unknown;suppliers?:unknown;clients?:unknown;source_refs?:unknown;rows?:unknown;flagged_rows?:unknown;kg?:unknown;first_date?:unknown;last_date?:unknown}
 ...
 try{
  ...
  const raw=await sql`with grouped as (
   select lower(btrim(lot_code)) lot_key,
    ...
    array_agg(distinct guide_number order by guide_number) filter(where nullif(btrim(guide_number),'') is not null) guides,
  ...
  const enriched=lots.map(row=>{
   const guides=asArray(row.guides),suppliers=asArray(row.suppliers),clients=asArray(row.clients)
   ...
   return {lotKey:String(row.lot_key??''),lotLabels:asArray(row.lot_labels),status,reasons,guides,suppliers,clients,rows:n(row.rows),flaggedRows:n(row.flagged_rows),kg:n(row.kg),firstDate:row.first_date??null,lastDate:row.last_date??null,sourceRefs:asArray(row.source_refs)}
  })
  ...
  const supplierConflictLots=enriched.filter(row=>row.reasons.includes('multiple_suppliers')).length
  const clientConflictLots=enriched.filter(row=>row.reasons.includes('multiple_clients')).length
  const multiGuideLots=enriched.filter(row=>row.reasons.includes('multiple_guides')).length
  return res.status(200).json({ok:true,schemaVersion:'pescamar.canonical-lot-reconciliation.v1',summary:{lotKeys:enriched.length,resolved:resolved.length,review:review.length,multiGuideLots,supplierConflictLots,clientConflictLots},filter:{status:requestedStatus,limit:requestedLimit,returned:Math.min(visible.length,requestedLimit)},lots:visible.slice(0,requestedLimit),governance:{mode:'evidence_only',writesHistorical:false,writesReceptions:false,writesInventory:false,writesParties:false,promotionAllowed:false,identityRule:'Un lote histórico sólo queda reconciliado cuando tiene exactamente una guía y un proveedor histórico, y no mezcla clientes. Esto no crea recepción, inventario, venta ni contraparte viva.',partyRule:'La reconciliación de lote no equivale a reconciliación de party. Las contrapartes maestras siguen requiriendo identidad exacta y verificada por separado.'}})
