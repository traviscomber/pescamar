...
const operationalRoles=new Set(["operations","quality","viewer"]);
const emailPattern=/^[^\s@]+@[^\s@]+\.[^\s@]+$/;

export default async function handler(request:Request,response:Response){
  response.setHeader("Cache-Control","no-store");
  try{
    const actor=await requireOperator(request,["admin","operations"]);
    if(!actor)return response.status(401).json({ok:false,error:"Sesión de Administración o Gerencia de Operaciones requerida"});
    ...
    if(request.method==="POST"){
      ...
      const rawPlantIds=Array.isArray(input.plantIds)?input.plantIds.map(value=>String(value).trim()).filter(Boolean):[];
      const plantIds=normalizePlantIds(rawPlantIds);
      if(rawPlantIds.length!==plantIds.length)return response.status(400).json({ok:false,error:"La selección contiene una planta no autorizada"});
      if(!isAdmin){
        if(!operationalRoles.has(role))return response.status(403).json({ok:false,error:"Gerencia de Operaciones solo puede crear Operaciones, Calidad o Lectura"});
        if(!plantIds.length||plantIds.some(id=>!actor.plantIds.includes(id)))return response.status(403).json({ok:false,error:"Solo puedes asignar plantas incluidas en tu propio alcance"});
        const existing=await getSql()`select role from operators where lower(email)=lower(${email}) limit 1`;
        const existingRole=Array.isArray(existing)?(existing[0] as {role?:string}|undefined)?.role:undefined;
        if(existingRole&& !operationalRoles.has(existingRole))return response.status(403).json({ok:false,error:"No puedes modificar una identidad de Administración o Finanzas"});
      }
