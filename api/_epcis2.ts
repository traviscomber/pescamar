...
export type EpcisSerializationDiagnostic={eventId:string;seafoodEventType:SeafoodEvent['type'];candidate:boolean;exportable:boolean;epcisType:'ObjectEvent'|'TransformationEvent'|null;cte:'receiving'|'transformation'|'shipping'|null;missing:string[];reason:string}
type QuantityElement={epcClass:string;quantity:number;uom:'KGM'}
type EpcisObjectEvent={type:'ObjectEvent';eventTime:string;eventTimeZoneOffset:string;action:'OBSERVE';bizStep:string;readPoint:{id:string};bizLocation:{id:string};quantityList:QuantityElement[];sourceList?:Array<{type:string;source:string}>;destinationList?:Array<{type:string;destination:string}>}
type EpcisTransformationEvent={type:'TransformationEvent';eventTime:string;eventTimeZoneOffset:string;bizStep:string;transformationID:string;readPoint:{id:string};bizLocation:{id:string};inputQuantityList:QuantityElement[];outputQuantityList:QuantityElement[]}
...
export function serializeSeafoodEventsToEpcis2(events:readonly SeafoodEvent[],resolveIdentity:(event:SeafoodEvent)=>EpcisIdentityContext){
 const eventList:EpcisEvent[]=[],diagnostics:EpcisSerializationDiagnostic[]=[]
 for(const event of events){
  const mapping=gdst2EventMappings.find(item=>item.seafoodEventType===event.type)
  if(!mapping||mapping.state!=='candidate'||!mapping.epcisType||!mapping.cte||!mapping.bizStep){diagnostics.push({eventId:event.id,seafoodEventType:event.type,candidate:false,exportable:false,epcisType:mapping?.epcisType??null,cte:mapping?.cte??null,missing:[],reason:mapping?.condition??'Este evento no tiene mapping EPCIS exportable.'});continue}
  ...
  if(mapping.epcisType==='ObjectEvent'){
   const product=text(identity.productClassUri),quantity=event.type==='reception'?finite(event.metrics.acceptedKg):event.type==='dispatch'?finite(event.metrics.dispatchedKg):null
   if(!product)missing.push('product class GS1')
   if(!quantity)missing.push('quantity kg')
   if(!text(identity.sourcePartyUri))missing.push('source party GS1')
   if(!text(identity.destinationPartyUri))missing.push('destination party GS1')
   ...
   const epcisEvent:EpcisObjectEvent={type:'ObjectEvent',eventTime:eventTime!,eventTimeZoneOffset:timezoneOffset(eventTime!),action:'OBSERVE',bizStep:bizStep(mapping.bizStep),readPoint:{id:readPoint!},bizLocation:{id:bizLocation!},quantityList:[{epcClass:product!,quantity:quantity!,uom:'KGM'}],sourceList:[{type:sourceType(),source:identity.sourcePartyUri!}],destinationList:[{type:destinationType(),destination:identity.destinationPartyUri!}]}
   eventList.push(epcisEvent);diagnostics.push({eventId:event.id,seafoodEventType:event.type,candidate:true,exportable:true,epcisType:mapping.epcisType,cte:mapping.cte,missing:[],reason:'Evento serializado desde evidencia e identidades explícitas.'});continue
  }
  const inputProduct=text(identity.inputProductClassUri),outputProduct=text(identity.outputProductClassUri),inputKg=finite(event.metrics.inputKg),outputKg=finite(event.metrics.outputKg)
 ...
 return {schemaVersion:EPCIS2_SERIALIZER_SCHEMA,epcisVersion:EPCIS2_VERSION,readOnly:true,externalWriteEnabled:false,document,events:eventList,diagnostics,summary:{inputEvents:events.length,candidates:diagnostics.filter(item=>item.candidate).length,exportable:eventList.length,blocked:diagnostics.filter(item=>item.candidate&&!item.exportable).length},boundary:{internalIdsAreNotGs1:true,partialEventsAllowed:false,claimGdstCapable:false}}
}
