...
import {buildLotEvidence,triageReviewRow,type LotEvidenceInput} from './_production-review-triage.js'
...
export default async function handler(req:Request,res:Response){
  ...
  try{
    ...
    const sql=getSql()
    const [rows,lotEvidenceRaw]=await Promise.all([
      ...
      sql`select lot_code,reception_date,process_date,production_date,data_quality_flags from historical_production_records where record_status='operational'`
    ])
    const lotEvidence=buildLotEvidence((Array.isArray(lotEvidenceRaw)?lotEvidenceRaw:[]) as LotEvidenceInput[])
    const reviewRows=(rows as ReviewRow[]).map(row=>{
      ...
      const normalizedRow={...row,context_rows:contextRows,data_quality_flags:flags}
      return {...normalizedRow,review_reasons:[...flags,...(contextRows>1?['non_unique_context']:[])],triage:triageReviewRow(normalizedRow)}
    })
    ...
    const focusCounts=reviewRows.reduce<Record<string,number>>((acc,row)=>{acc[row.triage.focus]=(acc[row.triage.focus]??0)+1;return acc},{})
    return res.status(200).json({
      ...
      governance:{mode:'evidence_only',writesHistorical:false,writesLive:false,derivedLotDate:'reference_only',rule:'Esta cola expone evidencia canónica y una fecha derivada del código de lote sólo como ayuda de revisión. No corrige, elimina, fusiona ni deduplica registros históricos. Cualquier rectificación requiere un flujo explícito y auditable que preserve el linaje original.'}
    })
  }catch(error){
