 ...
 try{
  ...
  const suppliers=[...names.entries()].map(([normalizedName,name])=>{
   ...
   const components:Component[]=[
    component('quality','Calidad',qualityScore,qualityScore==null?'Esperando evidencia de Grade A/color.':liveQuality!=null?`${histQualitySamples} registros históricos + ${liveQualitySamples} procesos live incorporados al score.`:`Rendimiento Grade A ${pct(gradeAYield)}${rejectionYield==null?'':` · rechazo ${pct(rejectionYield)}`}.`,qualitySource),
    component('yield','Rendimiento',yieldScore,yieldScore==null?'Sin rendimiento total suficiente.':liveYield!=null?`${histYieldSamples} registros históricos + ${liveYieldSamples} rendimientos live; live ${pct(nullable(l?.live_yield_ratio))}.`:`Rendimiento total histórico ${pct(nullable(h?.total_yield))} sobre ${histYieldSamples} registros.`,yieldSource),
   ...
   return {supplierId:l?.supplier_id??p?.supplier_id??null,supplier:name,score,label:labelFor(score,coverage,lots),confidence:confidenceFor(coverage,lots),coverage,lots,historicalLots,liveLots,receivedKg:n(h?.received_kg)+receivedLive,trendGradeAPoints:trend,components,zones:supplierZones,explanation,lastHistoricalDate:h?.last_date??null,identity:l?.supplier_id||p?.supplier_id?'canonical-or-exact-name':'historical-only',liveEvidence:{runs:liveRuns,qualitySamples:liveQualitySamples,yieldSamples:liveYieldSamples,controlledRuns}}
  ...
  return res.status(200).json({ok:true,method:{version:'supplier-score-v1.1-live-feedback',weights:WEIGHTS,rule:'Cada resultado live disponible entra al score inmediatamente y se mezcla con la evidencia histórica según cantidad real de muestras. La confianza sigue separada del valor del score. Calidad usa Grade A/color/rayos X; rendimiento usa kg salida; consistencia usa variación histórica de Grade A; cumplimiento usa guía vs recibido; rentabilidad usa contribución/kg relativa a pares; incidencias usa procesos live controlados con desviación.'},summary:{suppliers:suppliers.length,scored:scored.length,preferred,highConfidence},suppliers})
