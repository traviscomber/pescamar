...
type ZoneRow={supplier:string;zone:string;lots:string|number;received_kg:string|number;quality_samples:string|number;grade_a_yield:string|number|null;yield_samples:string|number;total_yield:string|number|null;mass_review_rows:string|number}
...
function confidenceFor(coverage:number,lots:number,massReviewRatio:number):Confidence{if(massReviewRatio>.25)return'baja';if(coverage>=70&&lots>=30&&massReviewRatio<=.05)return'alta';if(coverage>=50&&lots>=10&&massReviewRatio<=.15)return'media';return'baja'}
function labelFor(score:number|null,coverage:number,lots:number,confidence:Confidence){if(score==null||coverage<40||lots<3||confidence==='baja')return'Provisional';if(score>=85)return'Preferente';if(score>=75)return'Fuerte';if(score>=60)return'Estándar';return'Revisar'}
 ...
 try{
  ...
  const [historicalRaw,liveRaw,profitRaw,zonesRaw]=await Promise.all([
   sql`with raw as (
    ...
    select *,row_number() over(partition by supplier order by event_date desc nulls last,source_row desc) rn from base where grade_a is not null and not mass_review
   ),trend as (
    select supplier,
   ...
   select b.supplier,count(*) lots,sum(b.received_kg) received_kg,
    count(b.grade_a) filter(where not b.mass_review) quality_samples,
    sum(b.received_kg*b.grade_a) filter(where b.grade_a is not null and not b.mass_review)/nullif(sum(b.received_kg) filter(where b.grade_a is not null and not b.mass_review),0) grade_a_yield,
    count(b.rejection) filter(where not b.mass_review) rejection_samples,
    sum(b.received_kg*b.rejection) filter(where b.rejection is not null and not b.mass_review)/nullif(sum(b.received_kg) filter(where b.rejection is not null and not b.mass_review),0) rejection_yield,
    count(b.total_yield) filter(where not b.mass_review) yield_samples,
    sum(b.received_kg*b.total_yield) filter(where b.total_yield is not null and not b.mass_review)/nullif(sum(b.received_kg) filter(where b.total_yield is not null and not b.mass_review),0) total_yield,
    stddev_samp(b.grade_a) filter(where b.grade_a is not null and not b.mass_review) grade_a_stddev,
    avg(b.grade_a) filter(where b.grade_a is not null and not b.mass_review) grade_a_avg,
   ...
   sql`select p.id supplier_id,p.legal_name supplier,
    count(distinct r.id) filter(where lower(r.species) like '%eriz%') live_lots,
    count(u.id) live_runs,
   ...
   select supplier,zone,count(*) lots,sum(received_kg) received_kg,count(grade_a) filter(where not mass_review) quality_samples,
    ...
    count(total_yield) filter(where not mass_review) yield_samples,
    sum(received_kg*total_yield) filter(where total_yield is not null and not mass_review)/nullif(sum(received_kg) filter(where total_yield is not null and not mass_review),0) total_yield,
  ...
  const suppliers=[...names.entries()].map(([normalizedName,name])=>{
   ...
   const massReviewRows=n(h?.mass_review_rows),flaggedRows=n(h?.flagged_rows),guideRows=n(h?.guide_rows),pricedRows=n(h?.priced_rows),massReviewRatio=historicalLots>0?massReviewRows/historicalLots:0
   const components:Component[]=[
    component('quality','Calidad',qualityScore,qualityScore==null?'Esperando Grade A validado o controles live de color/rayos X.':liveQuality!=null?`${histQualitySamples} muestras canónicas válidas + ${liveQualitySamples} procesos live incorporados al score.`:`Grade A canónico ${pct(gradeAYield)}${rejectionYield==null?'':` · rechazo ${pct(rejectionYield)}`}.`,qualitySource),
    component('yield','Rendimiento',yieldScore,yieldScore==null?'Sin rendimiento físicamente reconciliado suficiente.':liveYield!=null?`${histYieldSamples} rendimientos canónicos válidos + ${liveYieldSamples} rendimientos live; live ${pct(nullable(l?.live_yield_ratio))}.`:`Rendimiento canónico ${pct(nullable(h?.total_yield))} sobre ${histYieldSamples} registros elegibles.`,yieldSource),
    component('consistency','Consistencia',consistencyScore,consistencyScore==null?'Se requieren al menos 5 muestras de Grade A comparables y válidas.':`Variabilidad de Grade A sobre ${histQualitySamples} muestras canónicas válidas.`,'historical'),
   ...
   const confidence=confidenceFor(coverage,lots,massReviewRatio)
   ...
   const evidenceNote=massReviewRows?` ${massReviewRows} filas canónicas quedan fuera del rendimiento por inconsistencia de masa; esto reduce la confianza, no la nota del proveedor.`:''
   const explanation=(score==null?'Todavía no existe evidencia suficiente para puntuar este proveedor.':`${strong?`Fortaleza: ${strong.label.toLowerCase()} ${strong.score?.toFixed(0)}.`:''}${weak&&weak.key!==strong?.key?` Principal brecha: ${weak.label.toLowerCase()} ${weak.score?.toFixed(0)}.`:''}${trend!=null&&Math.abs(trend)>=5?` Grade A ${trend>0?'sube':'baja'} ${Math.abs(trend).toFixed(1)} pp en los últimos registros comparables.`:''}`.trim())+evidenceNote
   return {supplierId:l?.supplier_id??p?.supplier_id??null,supplier:name,score,label:labelFor(score,coverage,lots,confidence),confidence,coverage,lots,historicalLots,liveLots,receivedKg:n(h?.received_kg)+receivedLive,trendGradeAPoints:trend,components,zones:supplierZones,explanation,lastHistoricalDate:h?.last_date??null,identity:l?.supplier_id||p?.supplier_id?'canonical-or-exact-name':'canonical-history-only',liveEvidence:{runs:liveRuns,qualitySamples:liveQualitySamples,yieldSamples:liveYieldSamples,controlledRuns},evidence:{canonicalLots:historicalLots,flaggedRows,massReviewRows,massValidatedPct:historicalLots?Number(((historicalLots-massReviewRows)/historicalLots*100).toFixed(1)):null,sourceFlaggedPct:historicalLots?Number((flaggedRows/historicalLots*100).toFixed(1)):null,guideCoveragePct:historicalLots?Number((guideRows/historicalLots*100).toFixed(1)):null,priceCoveragePct:historicalLots?Number((pricedRows/historicalLots*100).toFixed(1)):null,qualityEligibleRows:histQualitySamples,yieldEligibleRows:histYieldSamples}}
  ...
  const canonicalLots=suppliers.reduce((sum,item)=>sum+item.evidence.canonicalLots,0),massReviewRows=suppliers.reduce((sum,item)=>sum+item.evidence.massReviewRows,0)
  return res.status(200).json({ok:true,method:{version:'supplier-score-v1.2-canonical-evidence-gate',weights:WEIGHTS,rule:'El score histórico usa sólo fuentes marcadas como canónicas. Filas con inconsistencia de masa quedan fuera de calidad/rendimiento y sólo reducen confianza; nunca castigan directamente la nota del proveedor. Calidad mezcla Grade A canónico elegible con color/rayos X live; rendimiento mezcla sólo kg reconciliados; consistencia usa muestras Grade A válidas; cumplimiento usa guía vs recibido; rentabilidad usa contribución/kg relativa a pares; incidencias usa procesos live controlados.'},summary:{suppliers:suppliers.length,scored:scored.length,preferred,highConfidence,canonicalLots,massReviewRows},suppliers})
