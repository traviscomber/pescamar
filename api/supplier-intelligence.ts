...
type HistoricalRow={supplier:string;lots:string|number;received_kg:string|number;quality_samples:string|number;grade_a_yield:string|number|null;rejection_samples:string|number;rejection_yield:string|number|null;yield_samples:string|number;total_yield:string|number|null;grade_a_stddev:string|number|null;grade_a_avg:string|number|null;weight_deviation_ratio:string|number|null;recent_grade_a:string|number|null;prior_grade_a:string|number|null;first_date:string|null;last_date:string|null}
type LiveRow={supplier_id:string;supplier:string;live_lots:string|number;live_runs:string|number;live_grade_score:string|number|null;live_color_score:string|number|null;live_xray_score:string|number|null;live_yield_ratio:string|number|null;issue_runs:string|number}
...
type ZoneRow={supplier:string;zone:string;lots:string|number;received_kg:string|number;quality_samples:string|number;grade_a_yield:string|number|null;yield_samples:string|number;total_yield:string|number|null}
...
function qualityLive(row:LiveRow|undefined){if(!row||n(row.live_runs)<3)return null;const parts:[number|null,number][]=[[nullable(row.live_grade_score),.6],[nullable(row.live_color_score),.25],[nullable(row.live_xray_score),.15]],available=parts.filter((item):item is [number,number]=>item[0]!=null);if(!available.length)return null;const totalWeight=available.reduce((sum,item)=>sum+item[1],0);return available.reduce((sum,item)=>sum+item[0]*item[1],0)/totalWeight}
function labelFor(score:number|null,coverage:number,lots:number){if(score==null||coverage<40||lots<3)return 'Provisional';if(score>=85)return 'Preferente';if(score>=75)return 'Fuerte';if(score>=60)return 'Estándar';return 'Revisar'}
function confidenceFor(coverage:number,lots:number){if(coverage>=70&&lots>=30)return 'alta';if(coverage>=50&&lots>=10)return 'media';return 'baja'}
...
export default async function handler(req:Request,res:Response){
 ...
 try{
  ...
  const [historicalRaw,liveRaw,profitRaw,zonesRaw]=await Promise.all([
   sql`with base as (
    select coalesce(nullif(btrim(supplier_name),''),nullif(btrim(supplier_original),''),'Proveedor no identificado') supplier,
     ...
     case when yields ? 'total' then (yields->>'total')::numeric end total_yield,event_date,source_row
    from historical_production_records where record_status='operational'
   ), quality_ranked as (
    select *,row_number() over(partition by supplier order by event_date desc nulls last,source_row desc) rn from base where grade_a is not null
   ), trend as (
    select supplier,
   ...
   select b.supplier,count(*) lots,sum(b.received_kg) received_kg,
    count(b.grade_a) quality_samples,
    sum(b.received_kg*b.grade_a) filter(where b.grade_a is not null)/nullif(sum(b.received_kg) filter(where b.grade_a is not null),0) grade_a_yield,
    count(b.rejection) rejection_samples,
    sum(b.received_kg*b.rejection) filter(where b.rejection is not null)/nullif(sum(b.received_kg) filter(where b.rejection is not null),0) rejection_yield,
    count(b.total_yield) yield_samples,
    sum(b.received_kg*b.total_yield) filter(where b.total_yield is not null)/nullif(sum(b.received_kg) filter(where b.total_yield is not null),0) total_yield,
    stddev_samp(b.grade_a) filter(where b.grade_a is not null) grade_a_stddev,
    avg(b.grade_a) filter(where b.grade_a is not null) grade_a_avg,
   ...
   sql`with base as (
    ...
    from historical_production_records where record_status='operational'
   ) select supplier,zone,count(*) lots,sum(received_kg) received_kg,count(grade_a) quality_samples,
    sum(received_kg*grade_a) filter(where grade_a is not null)/nullif(sum(received_kg) filter(where grade_a is not null),0) grade_a_yield,
    count(total_yield) yield_samples,sum(received_kg*total_yield) filter(where total_yield is not null)/nullif(sum(received_kg) filter(where total_yield is not null),0) total_yield
  ...
  const suppliers=[...names.entries()].map(([normalizedName,name])=>{
   ...
   const historicalLots=n(h?.lots),liveLots=n(l?.live_lots),lots=historicalLots+liveLots
   ...
   const historicalQuality=histQualitySamples>=3&&gradeAYield!=null?100*gradeAYield*(rejectionYield==null?1:Math.max(0,1-rejectionYield)):null
   const liveQuality=qualityLive(l),qualityScore=n(l?.live_runs)>=5&&liveQuality!=null?liveQuality:historicalQuality
   const histYieldSamples=n(h?.yield_samples),historicalYield=histYieldSamples>=3&&nullable(h?.total_yield)!=null?100*n(h?.total_yield):null
   const liveYield=n(l?.live_runs)>=5&&nullable(l?.live_yield_ratio)!=null?100*n(l?.live_yield_ratio):null,yieldScore=liveYield??historicalYield
   ...
   const liveRuns=n(l?.live_runs),issueRuns=n(l?.issue_runs),incidentScore=liveRuns>=3?100*(1-Math.min(1,issueRuns/liveRuns)):null
   const components:Component[]=[
    component('quality','Calidad',qualityScore,qualityScore==null?'Esperando evidencia de Grade A/color.':n(l?.live_runs)>=5?`Calidad live sobre ${liveRuns} procesos de erizo.`:`Rendimiento Grade A ${pct(gradeAYield)}${rejectionYield==null?'':` · rechazo ${pct(rejectionYield)}`}.`,n(l?.live_runs)>=5?'live':qualityScore==null?'pending':'historical'),
    component('yield','Rendimiento',yieldScore,yieldScore==null?'Sin rendimiento total suficiente.':liveYield!=null?`Rendimiento live ${pct(nullable(l?.live_yield_ratio))}.`:`Rendimiento total histórico ${pct(nullable(h?.total_yield))} sobre ${histYieldSamples} registros.`,liveYield!=null?'live':yieldScore==null?'pending':'historical'),
    component('consistency','Consistencia',consistencyScore,consistencyScore==null?'Se requieren al menos 5 registros comparables.':`Variabilidad de Grade A medida sobre ${histQualitySamples} registros.`,'historical'),
    ...
    component('incidents','Incidencias',incidentScore,incidentScore==null?'Se activa con procesos live suficientes.':`${issueRuns} de ${liveRuns} procesos con al menos una desviación/hold.`,'live')
   ]
   ...
   const supplierZones=zones.filter(row=>key(row.supplier)===normalizedName).map(row=>({zone:row.zone,lots:n(row.lots),receivedKg:n(row.received_kg),gradeAYieldPct:nullable(row.grade_a_yield)==null?null:Number((n(row.grade_a_yield)*100).toFixed(1)),totalYieldPct:nullable(row.total_yield)==null?null:Number((n(row.total_yield)*100).toFixed(1))})).sort((a,b)=>(b.gradeAYieldPct??-1)-(a.gradeAYieldPct??-1)||b.receivedKg-a.receivedKg).slice(0,4)
   const ranked=available.slice().sort((a,b)=>(b.score??0)-(a.score??0)),strong=ranked[0],weak=ranked[ranked.length-1]
   const explanation=score==null?'Todavía no existe evidencia suficiente para puntuar este proveedor.':`${strong?`Fortaleza: ${strong.label.toLowerCase()} ${strong.score?.toFixed(0)}.`:''}${weak&&weak.key!==strong?.key?` Principal brecha: ${weak.label.toLowerCase()} ${weak.score?.toFixed(0)}.`:''}${trend!=null&&Math.abs(trend)>=5?` Grade A ${trend>0?'sube':'baja'} ${Math.abs(trend).toFixed(1)} pp en los últimos registros comparables.`:''}`.trim()
   return {supplierId:l?.supplier_id??p?.supplier_id??null,supplier:name,score,label:labelFor(score,coverage,lots),confidence:confidenceFor(coverage,lots),coverage,lots,historicalLots,liveLots,receivedKg:n(h?.received_kg)+receivedLive,trendGradeAPoints:trend,components,zones:supplierZones,explanation,lastHistoricalDate:h?.last_date??null,identity:l?.supplier_id||p?.supplier_id?'canonical-or-exact-name':'historical-only'}
  ...
  return res.status(200).json({ok:true,method:{version:'supplier-score-v1',weights:WEIGHTS,rule:'El score sólo pondera dimensiones con evidencia. Calidad usa Grade A/rechazo o clasificación live; rendimiento usa rendimiento total; consistencia usa variación de Grade A; cumplimiento usa guía vs recibido; rentabilidad usa contribución/kg relativa a pares; incidencias usa procesos live con desviación.'},summary:{suppliers:suppliers.length,scored:scored.length,preferred,highConfidence},suppliers})
 }catch(error){const message=error instanceof Error?error.message:'';return res.status(message.includes('historical_production_records')||message.includes('supplier_profitability')?503:500).json({ok:false,error:message.includes('historical_production_records')?'Falta inteligencia histórica para calcular Supplier Score':message.includes('supplier_profitability')?'Falta la proyección de rentabilidad por proveedor':'No fue posible calcular Supplier Intelligence'})}
}
