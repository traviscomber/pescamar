...
export default async function handler(req:Request,res:Response){
 ...
 if(!secret){
  console.info(JSON.stringify({event:'operational_health_cron',status:'disabled',reason:'cron_secret_missing',checkedAt:new Date().toISOString()}))
 ...
 try{
  ...
  console.info(JSON.stringify({event:'operational_health_cron',status:result.status,summary:result.summary,alertIds:result.alerts.map(alert=>alert.id),checkedAt:result.checkedAt,commit:result.deployment.commit}))
  ...
  console.error(JSON.stringify({event:'operational_health_cron',status:'broken',error:detail,checkedAt:new Date().toISOString()}))
