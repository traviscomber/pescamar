...
export const integrationAdapterRegistry:readonly IntegrationAdapterDefinition[]=[
  ...
  {id:'traceability-epcis-gdst',label:'GS1 EPCIS / GDST 2.0',direction:'northbound',protocol:'epcis/gdst',status:'foundation',mapsTo:'seafood.event.v1 → GDST 2.0 candidate event profiles',idempotency:'not_applicable',evidence:'Mapping candidato y GS1 identity registry foundation implementados. No se infiere que existan IDs confirmados; EPCIS JSON-LD, Query/Capture, Digital Link Resolver, master data y Capability Test oficial siguen pendientes. No se declara GDST Capable.',mutationEnabled:false},
] as const
