...
export default async function handler(req:Request,res:Response){
  ...
  try{
    ...
    const [operatorsRaw,receptionsRaw,evidenceRaw,qualityRaw,productionRaw,inventoryRaw,ordersRaw,dispatchesRaw,salesRaw,closesRaw,closeDaysRaw,endToEndRaw]=await Promise.all([
      ...
      sql`select plant_id,count(*) count,max(created_at) last_at from sales_orders where status<>'cancelled' group by plant_id`,
    ...
    return res.status(200).json({ok:true,plants,summary,gate:{version:'plant-uat-v3-linked-e2e',rule:'UAT exige roles y evidencia viva end-to-end enlazada sobre al menos un mismo lote: evidencia documental, Calidad, Producción, ubicación física y señal comercial vigente deben compartir reception_id. La revisión LIVE exige además al menos 3 fechas consecutivas con cierre operacional. LIVE nunca se infiere: requiere confirmación humana, cero P0/P1 y aceptación del responsable.'},disclaimer:'Gate basado en evidencia operacional enlazada. La presencia agregada de eventos en una planta no prueba un flujo E2E y la actividad comercial cancelada no cuenta. Los cierres consecutivos son evidencia de continuidad, no prueba automática de independencia del equipo técnico ni autorización LIVE.'})
