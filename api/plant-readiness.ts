...
export default async function handler(req:Request,res:Response){
  ...
  try{
    ...
    const sql=getSql()
    const [operatorsRaw,receptionsRaw,evidenceRaw,qualityRaw,productionRaw,inventoryRaw,ordersRaw,dispatchesRaw,salesRaw,closesRaw,closeDaysRaw]=await Promise.all([
    ...
    const plants=ids.filter(id=>visible(operator,id)).map(id=>{
      ...
      const users=[...role.values()].reduce((sum,item)=>sum+item.active,0),rec=receptions.get(id)?.count??0,ev=evidence.get(id)?.count??0,q=quality.get(id)?.count??0,p=production.get(id)?.count??0,inv=inventory.get(id)??0,commercial=(orders.get(id)?.count??0)+(dispatches.get(id)?.count??0)+(sales.get(id)?.count??0),close=closes.get(id)?.count??0,distinctCloseDays=closeDays.get(id)?.length??0,consecutiveCloseDays=consecutiveDays(closeDays.get(id)??[])
      const checks=[
    ...
    return res.status(200).json({ok:true,plants,summary,gate:{version:'plant-uat-v2-live-continuity',rule:'UAT exige roles y evidencia viva end-to-end. La revisión LIVE exige además al menos 3 fechas consecutivas con cierre operacional; esa señal no demuestra por sí sola ausencia de soporte técnico. LIVE nunca se infiere: requiere confirmación humana, cero P0/P1 y aceptación del responsable.'},disclaimer:'Gate basado en evidencia operacional. Los cierres consecutivos son evidencia de continuidad, no prueba automática de independencia del equipo técnico ni autorización LIVE.'})
