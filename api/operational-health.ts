...
function overall(checks:Check[]){if(checks.some(item=>item.status==='broken'))return 'broken' as const;if(checks.some(item=>item.status==='stuck'))return 'stuck' as const;if(checks.some(item=>item.status==='degraded'))return 'degraded' as const;return 'healthy' as const}
...
export default async function handler(req:Request,res:Response){
 ...
 const checks:Check[]=[],alerts:Alert[]=[]
 try{
  const sql=getSql()
  ...
  checks.push({key:'automation',label:'Supervisión automática',status:'degraded',detail:'El health es live bajo demanda; no hay cron de supervisión declarado en producción.',metrics:{scheduledHealthCheck:false}})
  alerts.push({id:'health-manual',severity:'info',domain:'platform',title:'Supervisión todavía bajo demanda',detail:'La app calcula salud en vivo. El siguiente paso es agregar notificación programada con autenticación de scheduler.',actionPath:'/observabilidad'})
  const state=overall(checks)
  return res.status(200).json({ok:true,status:state,summary:{checks:checks.length,healthy:checks.filter(item=>item.status==='healthy').length,degraded:checks.filter(item=>item.status==='degraded').length,stuck:checks.filter(item=>item.status==='stuck').length,broken:checks.filter(item=>item.status==='broken').length,critical:alerts.filter(item=>item.severity==='critical').length,warnings:alerts.filter(item=>item.severity==='warning').length},checks,alerts,method:{version:'operational-health-v1',staleProcessHours:24,canonicalFreshness:'manual-source-no-age-penalty',supportExpected:{blocks:89,observations:332,zeroObservationBlocks:1},scheduledHealthCheck:false},deployment:{environment:process.env.VERCEL_ENV??'local',commit:process.env.VERCEL_GIT_COMMIT_SHA?.slice(0,12)??null},checkedAt:new Date().toISOString()})
 }catch(error){
