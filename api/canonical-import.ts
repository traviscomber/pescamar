...
export default async function handler(req:Request,res:Response){
  ...
  try{
    ...
    if(kind==='production'){
      ...
      on conflict(source_file_hash,source_row) do update set record_status=excluded.record_status,event_date=excluded.event_date,reception_date=excluded.reception_date,process_date=excluded.process_date,production_date=excluded.production_date,guide_number=excluded.guide_number,supplier_original=excluded.supplier_original,supplier_name=excluded.supplier_name,extraction_zone=excluded.extraction_zone,guide_price_clp=excluded.guide_price_clp,process_site_original=excluded.process_site_original,lot_code=excluded.lot_code,guide_kg=excluded.guide_kg,received_kg=excluded.received_kg,difference_kg=excluded.difference_kg,quality_discount=excluded.quality_discount,grade_breakdown=excluded.grade_breakdown,yields=excluded.yields,client=excluded.client,observations=excluded.observations,data_quality_flags=excluded.data_quality_flags,raw_record=excluded.raw_record`
    }else if(kind==='ledger'){
      ...
      on conflict(source_file_hash,sheet_name,source_row) do update set event_date=excluded.event_date,description=excluded.description,inflow_clp=excluded.inflow_clp,outflow_clp=excluded.outflow_clp,balance_clp=excluded.balance_clp,data_quality_flags=excluded.data_quality_flags,raw_record=excluded.raw_record`
    }else if(kind==='stock'){
      ...
      on conflict(source_file_hash,sheet_name,source_row) do update set event_date=excluded.event_date,item=excluded.item,product_family=excluded.product_family,lot_reference=excluded.lot_reference,grade_breakdown=excluded.grade_breakdown,total_kg=excluded.total_kg,data_quality_flags=excluded.data_quality_flags,raw_record=excluded.raw_record`
      ...
      on conflict(source_file_hash,sheet_name,source_row) do update set event_date=excluded.event_date,bank=excluded.bank,sender=excluded.sender,amount_clp=excluded.amount_clp,data_quality_flags=excluded.data_quality_flags,raw_record=excluded.raw_record`
    }else{
      ...
      on conflict(source_file_hash,sheet_name,source_row) do update set pack_format=excluded.pack_format,box_number=excluded.box_number,lot_code=excluded.lot_code,production_date=excluded.production_date,weight_breakdown=excluded.weight_breakdown,total_kg=excluded.total_kg,notes=excluded.notes,data_quality_flags=excluded.data_quality_flags,raw_record=excluded.raw_record`
    }
    return res.status(200).json({ok:true,kind,fileName,fileHash,rows:rows.length,idempotent:true})
  }catch(error){
