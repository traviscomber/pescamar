...
export default async function handler(req:Request,res:Response){
 ...
 try{
  ...
  const [receptions,units]=await Promise.all([
   sql`select r.id,r.reception_number,r.plant_id,coalesce(p.legal_name,'Sin proveedor') supplier,r.species,r.quality_status,r.received_at from receptions r left join parties p on p.id=r.supplier_id where ${admin} or r.plant_id=any(${plantIds}::text[]) order by r.received_at desc limit 1000`,
  ...
  const missing=['receptions','packing_units','pallet_packing_units','pallets','parties'].some(name=>message.includes(name))
