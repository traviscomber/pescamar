...
function deterministicAnswer(question:string,lotControl:Record<string,unknown>|null,graph:Record<string,unknown>|null,measurements:VisualMeasurement[],comparison:Array<{index:number;deltaE76:number}>,seniorUrchin:boolean){
 ...
 if(lotControl){
  ...
  return [
   `Estado: ${stateLabel}. [lot_control]`,
   `Qué significa: ${meaning}`,
   `Atención: ${blockers[0]?`${blockers[0]}. [lot_control]`:'No hay un bloqueo determinístico visible en el control del lote; esto no prueba ausencia de otros riesgos. [lot_control]'}`,
   `Siguiente acción: ${nextAction?`${nextAction} [lot_control]`:'Dato faltante: el control del lote no entrega una siguiente acción.'}`,
   `Falta: ${unknowns.length?`${unknowns.join(' · ')} [lot_control]`:'Nada material adicional visible en lot_control; mantener revisión humana para decisiones de liberación, calidad o despacho. [lot_control]'}`,
   'Confianza: derived',
  ].join('\n')
 }
 if(seniorUrchin&&asksJapan)return ['Estado: Japón no evaluable sin lote.','Qué significa: no existe un Digital Twin de lote vinculado para verificar gates de proceso, calidad, frío y liberación.','Atención: no tratar una foto o una consulta general como autorización de exportación.','Siguiente acción: vincular el lote correcto y revisar su evidencia operacional.','Falta: identidad del lote y evidencia de sus gates de liberación.','Confianza: needs-human-validation'].join('\n')
 if(seniorUrchin&&measurements.length){const m=measurements[0];return [`Estado: Visual Twin disponible. [photo_observation]`,`Qué significa: LAB ${fmt(m.lMean)} / ${fmt(m.aMean)} / ${fmt(m.bMean)} · dispersión ${fmt(m.dispersion)}. [photo_observation]`,'Atención: la medición visual no prueba identidad, temperatura, inocuidad, Grade ni liberación. [photo_observation]','Siguiente acción: vincular el lote correcto para cruzar la observación visual con la operación.','Falta: identidad del lote y evidencia operacional asociada.','Confianza: needs-human-validation'].join('\n')}
 return ['Estado: Seafood AI operativa.','Qué significa: no hay evidencia determinística suficiente para responder esta consulta.','Atención: no convertir ausencia de datos en prueba de que un evento no ocurrió.','Siguiente acción: revisar las fuentes disponibles o seleccionar un lote.','Falta: evidencia operacional suficiente para la consulta.','Confianza: needs-human-validation'].join('\n')
}
