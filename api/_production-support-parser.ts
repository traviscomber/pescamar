...
const parserVersion='production-support-v1'
...
function rawCells(ws:ExcelJS.Worksheet,rows:number[],columns:number[]){const out:Record<string,unknown>={};for(const row of rows)for(const column of columns){const cell=ws.getCell(row,column);if(cell.value!==null&&cell.value!==undefined)out[cell.address]=rawValue(cell.value)}return out}
function findColumns(ws:ExcelJS.Worksheet,config:SheetConfig,start:number){let accepted:number|null=null,destined:number|null=null;for(let column=start+1;column<=Math.min(ws.columnCount,start+4);column++){const value=header(ws.getCell(config.headerRow,column).value);if(accepted==null&&value.includes('acept'))accepted=column;if(destined==null&&(value.includes('destin')||value.includes('rga')||value==='d'))destined=column}return {accepted,destined}}
function metadata(ws:ExcelJS.Worksheet,config:SheetConfig,start:number){
 const columns=[] as number[]
 // Every support chain occupies three columns: guide, accepted and destination/RGA.
 // start+3 is the next chain's guide column and must never contribute metadata.
 for(let column=Math.max(1,start-1);column<=Math.min(ws.columnCount,start+2);column++)columns.push(column)
...
export function parseProductionSupport(workbook:ExcelJS.Workbook){const rows:ProductionSupportRow[]=[];for(const config of CONFIGS){const ws=workbook.getWorksheet(config.sheetName);if(!ws)continue;for(let start=1;start<=ws.columnCount;start++){if(header(ws.getCell(config.headerRow,start).value)!=='kilosguia')continue;const {accepted,destined}=findColumns(ws,config,start);if(accepted==null)continue;const meta=metadata(ws,config,start),flags:string[]=[];if(!meta.guideNumber)flags.push('missing_guide_number');if(!meta.lotReference)flags.push('missing_lot_reference');for(let sourceRow=config.gradeStart;sourceRow<=config.gradeEnd;sourceRow++){const gradeCode=textValue(ws.getCell(sourceRow,config.gradeLabelCol).value);if(!gradeCode)continue;const guideKg=numberValue(ws.getCell(sourceRow,start).value),acceptedKg=numberValue(ws.getCell(sourceRow,accepted).value),destinedKg=destined==null?null:numberValue(ws.getCell(sourceRow,destined).value);if((guideKg??0)===0&&(acceptedKg??0)===0&&(destinedKg??0)===0)continue;const rowFlags=[...flags];if(guideKg==null&&acceptedKg!=null)rowFlags.push('accepted_without_grade_guide');rows.push({sheetName:config.sheetName,sourceBlock:start,sourceRow,familyKey:config.familyKey,eventDate:meta.eventDate,supplierName:config.supplierName,processSite:config.processSite,guideNumber:meta.guideNumber,lotReference:meta.lotReference,gradeCode,guideKg,acceptedKg,destinedKg,notes:meta.notes,dataQualityFlags:rowFlags,rawRecord:rawCells(ws,[...config.metadataRows,config.headerRow,sourceRow,...config.noteRows],[config.gradeLabelCol,...meta.columns,accepted,...(destined==null?[]:[destined])])})}}}return {parserVersion,rows,blocks:new Set(rows.map(row=>`${row.sheetName}|${row.sourceBlock}`)).size,bySheet:Object.fromEntries(CONFIGS.map(config=>[config.sheetName,rows.filter(row=>row.sheetName===config.sheetName).length]))}}
