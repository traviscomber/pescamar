...
async function queuePrint(input:Input,res:Response,operator:SessionOperator){
 ...
 if(!uuid.test(packingUnitId)||productLabelId&&!uuid.test(productLabelId)||!uuid.test(labelTemplateId)||!uuid.test(printerDeviceId)||sourceJobId&&!uuid.test(sourceJobId)||!copies||copies>20||idempotencyKey.length<8)return res.status(400).json({ok:false,error:'Solicitud de impresión inválida'})
 const sql=getSql()
 ...
 let label:null|Record<string,unknown>=null
 if(productLabelId){
  const row=first<Record<string,unknown>&{reception_id?:string}>(await sql`select id,reception_id,label_code,product,grade,lot_code,net_kg,destination,packing_format,process_type,species,status from product_labels where id=${productLabelId}::uuid limit 1`)
  if(!row||row.reception_id!==packing.reception_id)return res.status(409).json({ok:false,error:'Etiqueta no corresponde al lote de esta unidad'})
  label=row
 }
 if(sourceJobId){
  const source=first<{id:string;plant_id:string;packing_unit_id:string}>(await sql`select id,plant_id,packing_unit_id from label_print_jobs where id=${sourceJobId}::uuid limit 1`)
  if(!source||source.plant_id!==packing.plant_id||source.packing_unit_id!==packing.id)return res.status(409).json({ok:false,error:'Trabajo fuente de reimpresión inválido'})
 }
 ...
 const existing=first<{id:string;packing_unit_id:string;label_template_id:string;printer_device_id:string;copies:number;status:string}>(await sql`select id,packing_unit_id,label_template_id,printer_device_id,copies,status from label_print_jobs where plant_id=${packing.plant_id} and idempotency_key=${idempotencyKey} limit 1`)
 if(existing){
  const same=existing.packing_unit_id===packingUnitId&&existing.label_template_id===labelTemplateId&&existing.printer_device_id===printerDeviceId&&Number(existing.copies)===copies
