...
export async function loadPhysicalLineageEvents(receptionId:string,siteId:string|null,organization:OrganizationRef):Promise<SeafoodEvent[]>{
  ...
  const [processRaw,packingRaw,palletRaw,coldRaw]=await Promise.all([
    ...
    sql`select distinct p.id,p.pallet_code,p.status,p.product,p.species,p.grade,p.destination,p.box_count,p.net_kg,p.created_by_operator_id,p.closed_by_operator_id,p.created_at,p.closed_at,coalesce(p.closed_at,p.created_at) sort_at,(select count(*) from pallet_packing_units x join packing_units ux on ux.id=x.packing_unit_id where x.pallet_id=p.id and x.removed_at is null and ux.reception_id=${receptionId}::uuid) linked_units from pallets p join pallet_packing_units ppu on ppu.pallet_id=p.id and ppu.removed_at is null join packing_units pu on pu.id=ppu.packing_unit_id where pu.reception_id=${receptionId}::uuid order by sort_at`,
    sql`select distinct cl.id load_id,cr.id run_id,cr.run_code,cr.status,cr.min_allowed_c,cr.max_allowed_c,cr.observed_min_c,cr.observed_max_c,cr.last_observed_c,cr.observation_count,cr.deviation_count,cr.started_by_operator_id,cr.completed_by_operator_id,cr.started_at,cr.completed_at,cr.evidence_url,cr.notes,ca.id asset_id,ca.code asset_code,ca.name asset_name,cl.pallet_id,cl.reception_id load_reception_id,cl.added_at,cl.released_at,coalesce(cr.completed_at,cr.started_at,cl.added_at) sort_at from cold_run_loads cl join cold_runs cr on cr.id=cl.run_id left join cold_assets ca on ca.id=cr.asset_id where cl.reception_id=${receptionId}::uuid or cl.pallet_id in (select distinct ppu.pallet_id from pallet_packing_units ppu join packing_units pu on pu.id=ppu.packing_unit_id where ppu.removed_at is null and pu.reception_id=${receptionId}::uuid) order by sort_at`,
  ])
  ...
  for(const row of rows(coldRaw)){
    const runId=String(row.run_id),loadId=String(row.load_id)
    events.push(seafoodEvent({id:`cold:${runId}:${loadId}`,siteId,lotId:receptionId,type:'cold',occurredAt:text(row.completed_at)??text(row.started_at)??text(row.added_at),title:`Frío · ${text(row.run_code)??runId.slice(0,8)}`,detail:text(row.asset_name)??text(row.notes),actor:text(row.completed_by_operator_id)??text(row.started_by_operator_id),metrics:{coldRunId:runId,coldLoadId:loadId,runCode:text(row.run_code),status:text(row.status),assetId:text(row.asset_id),assetCode:text(row.asset_code),assetName:text(row.asset_name),palletId:text(row.pallet_id),loadReceptionId:text(row.load_reception_id),minAllowedC:numberOrNull(row.min_allowed_c),maxAllowedC:numberOrNull(row.max_allowed_c),observedMinC:numberOrNull(row.observed_min_c),observedMaxC:numberOrNull(row.observed_max_c),lastObservedC:numberOrNull(row.last_observed_c),observationCount:numberOrNull(row.observation_count),deviationCount:numberOrNull(row.deviation_count),evidenceUrl:text(row.evidence_url),releasedAt:text(row.released_at)},source:{entityType:'cold_run_load',entityId:loadId}},organization))
  }
