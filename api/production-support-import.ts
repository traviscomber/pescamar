...
export default async function handler(request:Request,response:Response){
 ...
 try{
  ...
  if(!parsed.rows.length)return response.status(422).json({ok:false,error:'No se encontraron filas utilizables en Isla Guafo, Diaz termiando o Cesar'})
  ...
  const flagged=parsed.rows.filter(row=>row.dataQualityFlags.length>0).length
  return response.status(200).json({ok:true,fileName,fileHash,parserVersion:parsed.parserVersion,blocks:parsed.blocks,rows:parsed.rows.length,flagged,bySheet:parsed.bySheet,idempotent:true,writesLive:false})
 }catch(error){console.error('production_support_import_failed',error);const message=error instanceof Error?error.message:'';if(message.includes('canonical_production_support_rows')||message.includes('42P01'))return response.status(503).json({ok:false,error:'Falta aplicar la migración 030 de evidencia auxiliar de producción'});return response.status(message.includes('DATABASE_URL')?503:500).json({ok:false,error:message.includes('DATABASE_URL')?'Base de datos no conectada':'No fue posible publicar las hojas auxiliares de producción'})}
}
