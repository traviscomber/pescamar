...
export default async function handler(request:Request,response:Response){
 ...
 try{
  ...
  return response.status(200).json({ok:true,fileName,fileHash,parserVersion:parsed.parserVersion,blocks:parsed.blocks,rows:parsed.rows.length,flagged,flaggedBlocks,bySheet:parsed.bySheet,blocksBySheet:parsed.blocksBySheet,idempotent:true,immutable:true,writesLive:false})
