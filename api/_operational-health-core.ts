...
import {assessCommunicationsReadiness} from './_communications-health.js'
...
export async function calculateOperationalHealth(options:{scheduledHealthCheck:boolean}):Promise<OperationalHealthResult>{
 ...
 if(!registry?.whatsapp_channels||!registry?.whatsapp_messages||!registry?.communication_insights){
  ...
  const configured=Boolean(process.env.WHATSAPP_WEBHOOK_SECRET),critical=num(insight?.critical_pending),pending=num(insight?.pending),messages=num(message?.messages),messages24h=num(message?.messages_24h),activeChannels=num(channel?.active_channels),interpretedChannels=num(channel?.interpreted_channels)
  const readiness=assessCommunicationsReadiness({webhookConfigured:configured,activeChannels,interpretedChannels,criticalPending:critical,messages,messages24h,pendingInsights:pending})
  const status:Health=readiness.status
  checks.push({key:'communications',label:'WhatsApp Intelligence',status,detail:readiness.detail,metrics:{webhookConfigured:configured,activeChannels,interpretedChannels,messages,messages24h,pendingInsights:pending,criticalPending:critical,lastIngestedAt:iso(message?.last_ingested_at)}})
  if(readiness.warning)alerts.push({...readiness.warning,severity:'warning',domain:'communications',actionPath:'/comunicaciones'})
