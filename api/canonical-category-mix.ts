...
export default async function handler(request:Request,response:Response){
 ...
 try{
  ...
  const suppliers=(Array.isArray(supplierRaw)?supplierRaw:[]).map(item=>{const {name,...row}=shape(item,'supplier');return {supplier:name,...row}})
  const sites=(Array.isArray(siteRaw)?siteRaw:[]).map(item=>{const {name,...row}=shape(item,'process_site');return {site:name,...row}})
