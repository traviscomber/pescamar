...
async function listPending(response:Response,operator:SessionOperator){
  ...
  const finance=['admin','finance'].includes(operator.role)
  const receptionRole=['admin','operations','quality'].includes(operator.role)
  ...
  const rows=(await getSql()`
    ...
    where cr.status='pending' and ${finance}
    union all
    ...
    where s.status='pending' and ${finance} and (${admin} or r.plant_id=any(${plantIds}::text[]))
...
async function decide(body:unknown,response:Response,operator:SessionOperator){
  ...
  let changed:unknown
  if(entityType==='credit_request')changed=await sql`with changed as (update credit_requests set status=${decision}::workflow_status,approved_at=case when ${decision}='approved' then now() else null end where id=${entityId}::uuid and status='pending' returning id,account_id,amount_clp),movement as (insert into credit_movements(account_id,credit_request_id,kind,amount_clp,comment,created_by) select account_id,id,'advance',amount_clp,${comment},${operator.fullName} from changed where ${decision}='approved' on conflict(credit_request_id) where kind='advance' do nothing) insert into approval_actions(entity_type,entity_id,action,comment,acted_by) select ${entityType},id,${decision}::workflow_status,${comment},${operator.fullName} from changed returning entity_id`
  else if(entityType==='reception')changed=await sql`with changed as (update receptions set status=${decision}::workflow_status where id=${entityId}::uuid and status='pending' and (${admin} or plant_id=any(${plantIds}::text[])) returning id) insert into approval_actions(entity_type,entity_id,action,comment,acted_by) select ${entityType},id,${decision}::workflow_status,${comment},${operator.fullName} from changed returning entity_id`
  else if(decision==='rejected')changed=await sql`with changed as (update settlements s set status='rejected',updated_at=now() from receptions r where s.id=${entityId}::uuid and s.status='pending' and r.id=s.reception_id and (${admin} or r.plant_id=any(${plantIds}::text[])) returning s.id) insert into approval_actions(entity_type,entity_id,action,comment,acted_by) select ${entityType},id,'rejected'::workflow_status,${comment},${operator.fullName} from changed returning entity_id`
  else changed=await sql`with candidate as (select s.*,r.supplier_id,p.legal_name,greatest(s.gross_amount_clp-s.other_deductions_clp,0)::bigint as payable,ca.id as account_id,coalesce(cb.balance_clp,0)::bigint as balance_clp,cr.recovery_kind,cr.recovery_value from settlements s join receptions r on r.id=s.reception_id join parties p on p.id=r.supplier_id left join lateral(select a.id from credit_accounts a join parties ap on ap.id=a.party_id where a.party_id=r.supplier_id or lower(trim(ap.legal_name))=lower(trim(p.legal_name)) order by(a.party_id=r.supplier_id) desc limit 1)ca on true left join credit_account_balances cb on cb.account_id=ca.id left join lateral(select q.recovery_kind,q.recovery_value from credit_requests q where q.account_id=ca.id and q.status='approved' order by q.approved_at desc nulls last limit 1)cr on true where s.id=${entityId}::uuid and s.status='pending' and (${admin} or r.plant_id=any(${plantIds}::text[]))),calculated as(select *,least(balance_clp,payable,case recovery_kind when 'percentage' then round(payable*recovery_value/100)::bigint when 'fixed_amount' then recovery_value::bigint when 'full_balance' then balance_clp else 0 end)::bigint as recovery from candidate),changed as(update settlements s set status='approved',credit_recovery_clp=c.recovery,net_amount_clp=c.payable-c.recovery,settled_at=now(),approved_by=${operator.fullName},updated_at=now(),calculation_snapshot=s.calculation_snapshot||jsonb_build_object('creditBalanceBeforeClp',c.balance_clp,'creditRecoveryClp',c.recovery,'netAmountClp',c.payable-c.recovery,'approvalComment',${comment}) from calculated c where s.id=c.id returning s.id,c.account_id,c.recovery),movement as(insert into credit_movements(account_id,settlement_id,kind,amount_clp,comment,created_by) select account_id,id,'recovery',recovery,${comment},${operator.fullName} from changed where account_id is not null and recovery>0 on conflict(settlement_id) where kind='recovery' do nothing) insert into approval_actions(entity_type,entity_id,action,comment,acted_by) select ${entityType},id,'approved'::workflow_status,${comment},${operator.fullName} from changed returning entity_id`
  if(!Array.isArray(changed)||!changed.length)return response.status(404).json({ok:false,error:'La entidad ya fue procesada o está fuera de tu alcance'})
