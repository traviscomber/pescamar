...
type SupportRow={sheet_name:unknown;source_block:unknown;source_row:unknown;family_key:unknown;event_date:unknown;supplier_name:unknown;process_site:unknown;guide_number:unknown;lot_reference:unknown;grade_code:unknown;guide_kg:unknown;accepted_kg:unknown;destined_kg:unknown;notes:unknown;data_quality_flags:unknown}
...
export default async function handler(request:Request,response:Response){
 ...
 try{
  ...
  let supportRows:SupportRow[]=[],supportStatus:'ready'|'not_imported'|'migration_required'='ready'
  try{
   const supportRaw=await sql`select sheet_name,source_block,source_row,family_key,event_date,supplier_name,process_site,guide_number,lot_reference,grade_code,guide_kg,accepted_kg,destined_kg,notes,data_quality_flags
    from canonical_production_support_rows
    where parser_version='production-support-v1'
  ...
  return response.status(200).json({ok:true,method:{version:'rollforward-linkage-v2-support-evidence',rule:'La fila principal conserva recepción, lote, guía, proveedor y planta. Las hojas Isla Guafo, Diaz termiando y Cesar se guardan como evidencia auxiliar por bloque y grado. Un vínculo automático sólo se acepta si guía y/o prefijo de lote identifican una única fila sin contradicción. Conflictos y ausencias quedan para revisión humana. Kilos guía, aceptados o destinados/RGA de las hojas auxiliares no se publican como rendimiento.'},summary:{rows:observations.length,chains:chains.length,uniqueLots,uniqueGuides,receivedKg:Number(receivedKg.toFixed(1)),destinationRows,destinationCoveragePct:pct(destinationRows,observations.length),gradeRows,gradeCoveragePct:pct(gradeRows,observations.length),gradeObservationCount,unresolvedDestinationRows:observations.length-destinationRows},support:{status:supportStatus,parserVersion:'production-support-v1',rows:supportRows.length,blocks:supportBlocks.length,autoLinkedBlocks,exactBoth,guideOnly,lotOnly,conflicts,ambiguous,unmatched,matchCoveragePct:pct(autoLinkedBlocks,supportBlocks.length),matchedMainRows,mainCoveragePct:pct(matchedMainRows,observations.length)},supportBlocks,chains})
