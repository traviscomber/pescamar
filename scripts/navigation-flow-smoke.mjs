...
check(shell.includes('{to:"/ordenes-venta",label:"Órdenes",step:1}')&&shell.includes('{to:"/despachos-ventas",label:"Despacho",step:2}')&&shell.includes('{to:"/liquidaciones",label:"Liquidación",step:3}'),'commercial must expose one ordered three-stage execution flow')
check(!shell.includes('{to:"/proveedores-clientes",label:"Proveedores",step:'),'partner master data must not compete in the daily commercial execution flow')
