...
check(modules.includes("{to:'/uni',label:'Revisión visual (Uni)'")&&app.includes('<Route path="/uni"')&&app.includes('<Route path="/edgevision" element={<Navigate to="/uni" replace/>}/>'),'specialized visual review must remain reachable from administration with legacy redirect')
...
console.log('Navigation flow smoke PASS: four localized workspaces, plant-centric nine-stage flow, separated administration, specialist visual review access and operating responsibility model verified')
