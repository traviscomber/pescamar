...
check(shell.includes('className={`settings-link ${workspace==="admin"?"active":""}`}')&&shell.includes("t('nav.admin')"),'administration must remain a separate low-frequency destination')
...
check(shell.includes("workspace==\"operation\"&&plantContextId?t('shell.plantFlow')")&&shell.includes("workspace==\"commercial\"?t('shell.commercialFlow')")&&shell.includes("workspace==\"intelligence\"?t('shell.intelligenceFlow')"),'workspace flows must expose localized process semantics for assistive technology')
check(shell.includes('aria-label={tabsLabel}')&&shell.includes('workspace-step-index'),'workspace flows must expose accessible labels and stage numbers')
