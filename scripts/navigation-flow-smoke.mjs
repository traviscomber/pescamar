...
check(roleExperience.includes("primaryPaths:['/','/pescamar-ia','/rentabilidad','/ordenes-venta']")&&roleExperience.includes('isCeoOperator'),'CEO experience must remain identity-derived while preserving admin authorization')
check(shell.includes("operationsView=operator?.role==='operations'")&&shell.includes('cleanExecutiveView=operationsView||ceoView')&&shell.includes('!cleanExecutiveView?<>'),'operations and CEO views must suppress low-frequency secondary navigation while retaining those destinations for other authorized roles')
...
check(shell.includes('aria-label={workspaceNavLabel}')&&shell.includes("aria-label={t('nav.consultSettings')}")&&shell.includes("workspaceNavLabel=ceoView?t('nav.executive'):t('nav.dailyWork')"),'primary executive/daily and conditional secondary navigation groups must expose explicit assistive labels')
...
console.log('Navigation flow smoke PASS: role-first daily surfaces, CEO and operations executive focus, executive Seafood AI access, quality-specific primary workspace, detailed plant and sales flows, conditional secondary admin access and minimum-team responsibility model verified')
