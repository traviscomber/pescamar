...
for(const token of ["intended_use==='defect_variability'","reviewPriority","pending_quality_review","visionTest","analyzedPending","qualityKnowledge","latest_human_quality_review_per_reference","visionQualityComparison","same_external_reference_with_persisted_vision_test_and_latest_human_quality_review","compatibleAccepted","compatibleRejected","cautionAccepted","cautionRejected","derivedKnowledge:qualityKnowledge!==null","visionComparison:visionQualityComparison!==null","req.method==='POST'","sea_urchin_external_reference_reviews","automaticTraining:false","rejectionReasons"]){if(!endpoint.includes(token))fail.push(`review contract missing ${token}`)}
for(const token of ['Calidad enseña al sistema','Vision propone. Calidad decide.','Análisis Vision:','Decisión humana requerida.','Conocimiento observado','Lo que Calidad ya enseñó','no es accuracy del modelo','No se muestra esta sección hasta existir feedback humano real.','Vision × Calidad','Matriz observada de resultado humano','la cautela no equivale a error','Autoridad final: Calidad.','Aprobar','Rechazar','Color fuera de objetivo','Daño visual','no reentrena automáticamente']){if(!queue.includes(token))fail.push(`review queue UI missing ${token}`)}
...
console.log(`Sea urchin external reference smoke PASS · ${ids.length} unlabeled references + observed Vision-to-Quality comparison`)
