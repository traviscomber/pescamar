...
for(const token of ["intended_use==='defect_variability'","reviewPriority","Prioridad derivada sólo desde contexto/provenance","req.method==='POST'","sea_urchin_external_reference_reviews","automaticTraining:false","rejectionReasons"]){if(!endpoint.includes(token))fail.push(`review contract missing ${token}`)}
for(const token of ['Calidad enseña al sistema','Vision propone contexto. Calidad define la etiqueta.','Aprobar','Rechazar','Color fuera de objetivo','Daño visual','no reentrena automáticamente']){if(!queue.includes(token))fail.push(`review queue UI missing ${token}`)}
...
console.log(`Sea urchin external reference smoke PASS · ${ids.length} unlabeled references + human Quality review flow`)
