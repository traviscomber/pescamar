...
assert(pageSource.includes("'/api/lot-lineage?receptionId='"),'lineage UI must retain the canonical live lineage endpoint')
assert(pageSource.includes("'/api/historical-lineage?recordId='"),'lineage UI must consume the canonical historical lineage endpoint')
assert(pageSource.includes('Live lineage')&&pageSource.includes('Histórico canónico'),'lineage UI must expose explicit live and canonical historical modes')
assert(pageSource.includes('CANONICAL HISTORICAL · READ ONLY'),'lineage UI must label historical lineage as read-only')
assert(pageSource.includes('No modifica ni completa artificialmente la operación live'),'lineage UI must not promote historical evidence to live operation')
...
console.log('Seafood lineage smoke PASS: versioned dual-mode Event Graph, authenticated boundaries, canonical historical provenance, read-only/live separation, Vision provenance and OS navigation verified')
