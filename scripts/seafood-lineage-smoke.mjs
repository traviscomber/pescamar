...
assert(lineageSource.includes('ai_provider,ai_model,ai_confidence'),'live lineage must retrieve persisted reception AI provenance')
assert(lineageSource.includes("aiEvidenceClass:aiProvider||aiModel||aiConfidence!=null?'ai_extraction':'persisted_evidence'"),'persisted evidence must distinguish AI extraction from ordinary persisted evidence')
assert(lineageSource.includes('aiProvider,aiModel,aiConfidence'),'AI provider, model and confidence must survive into Event Graph metrics')
...
assert(pageSource.includes('<option value="live">Actual</option>')&&pageSource.includes('<option value="historical">Reportes por período</option>'),'trazabilidad UI must expose simple current and historical-report modes')
assert(pageSource.includes('Reporte del período · solo lectura.'),'trazabilidad UI must label historical lineage as read-only')
assert(pageSource.includes('No modifica ni completa la operación actual.'),'trazabilidad UI must not promote historical evidence to live operation')
...
assert(pageSource.includes("provenanceLabels:Record<ProvenanceKind,string>={human:'Humano',ai:'IA',derived:'Derivado',unknown:'Origen no atribuido'}"),'lineage UI must expose evidence-safe provenance labels')
assert(pageSource.includes("m.aiEvidenceClass==='ai_extraction'||event.type==='vision'"),'lineage UI must classify AI only from explicit Vision provenance')
assert(pageSource.includes("if(event.actor)return {kind:'human'"),'lineage UI must classify human evidence only when an actor is attributable')
assert(pageSource.includes("return {kind:'unknown'"),'missing attribution must remain unknown rather than inferred human or derived evidence')
assert(pageSource.includes('Humano = actor/revisión atribuible')&&pageSource.includes('Origen no atribuido = evidencia insuficiente para clasificar'),'operator legend must explain provenance boundaries explicitly')
assert(pageSource.includes('lineage-provenance ${source.kind}'),'each live event must render its provenance badge')
...
console.log('Seafood lineage smoke PASS: versioned dual-mode Event Graph, persisted Vision provenance, evidence-safe human/AI/derived attribution, operational intelligence, authenticated boundaries, canonical historical provenance and read-only/live separation verified')
