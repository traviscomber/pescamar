...
assert(focus.includes("type SensorState='not_configured'|'ready_for_test'|'observed'")&&focus.includes("status?.state==='observed'")&&focus.includes("status?.state==='ready_for_test'")&&focus.includes("value:'Automáticas'")&&focus.includes("value:'Listo para prueba'")&&focus.includes("value:'Registro manual'"),'daily cold-chain UI must expose truthful observed, test-ready and unconfigured activation states with concise operator labels')
assert(focus.includes("fetch('/api/cold-sensor-status'"),'daily cold-chain UI must read server-observed sensor status')
assert(focus.includes('No hay sensores configurados. La operación puede seguir registrando temperatura manualmente.'),'daily cold-chain UI must keep manual physical capture available when telemetry is not configured')
assert(model.includes("stage:'Cadena de frío',state:'ready'")&&model.includes('La lectura manual sigue siendo una captura física válida.'),'operating model must treat a manual temperature reading as legitimate physical capture within the minimum flow')
assert(model.includes('Conectar sensores reduce carga, pero no bloquea el flujo mínimo.'),'sensor hardware must remain an optional workload reduction rather than a false software completion gate')
