...
assert(focus.includes("type SensorState='not_configured'|'ready_for_test'|'observed'")&&focus.includes("status?.state==='observed'")&&focus.includes("status?.state==='ready_for_test'")&&focus.includes('Lecturas automáticas activas')&&focus.includes('Listo para prueba')&&focus.includes('Sin sensores configurados'),'daily cold-chain UI must expose truthful observed, test-ready and unconfigured activation states')
assert(focus.includes("fetch('/api/cold-sensor-status'"),'daily cold-chain UI must read server-observed sensor status')
assert(focus.includes('La operación puede seguir registrando la temperatura manualmente.'),'daily cold-chain UI must keep manual physical capture available when telemetry is not configured')
