...
assert(event.includes('organizationId:organization.organizationId'),'event envelope must inherit the organization context supplied by the authenticated caller')
assert(event.includes('system:organization.sourceSystem'),'event provenance source system must inherit the same resolved organization context')
...
assert(lineage.includes('},organization))'),'lineage must propagate the resolved organization into every Seafood Event envelope')
...
console.log('Organization context smoke PASS: client/server alignment, auth/login fail-closed assertions, resolved Event Graph provenance, truthful isolation state and Implementation 02 gates verified')
