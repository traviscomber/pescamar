...
assert(production.includes("priority.receptionId?<button className=\"button primary\"")&&production.includes("blocked?<Link className=\"button primary\" to=\"/planificacion\">{text('Resolver bloqueo','Resolve blocker')}</Link>"),'production must open executable lots directly and expose planning only as an exception/blocker path')
assert(production.includes("priority?actionLabel(priority.action):text('Sin producción pendiente','No production pending')")&&production.includes("!hasLots?<Link className=\"button primary\" to=\"/recepciones\">{text('Ir a recepciones','Go to receptions')}</Link>:null"),'production empty state must keep the operator on the next real action rather than force plan review')
...
assert(inventory.includes('const primary=totals.unlocated>0')&&inventory.includes(':firstBlocked')&&inventory.includes("text('Hay producto sin ubicación registrada.'")&&inventory.includes("text('Revisar lote','Review lot')"),'inventory must escalate physical location gaps first and release blockers second')
...
assert(salesOrders.includes("mode==='allocate'?(isEs?'Confirmar reserva':'Confirm reservation'):(isEs?'Confirmar':'Confirm')"),'commercial economic commitment must require explicit human confirmation')
...
assert(today.includes("const suggestedOwner=(path:string)=>")&&today.includes("t('home.ownerCommercial'):t('home.ownerOperations')"),'Today must route each priority to a localized minimum-team responsibility without inventing an individual assignment')
assert(today.includes('owner:suggestedOwner(item.path)')&&today.includes('owner:suggestedOwner(to)')&&today.includes("const assistantFirst=operator?.role==='operations'||ceoView"),'Today must preserve functional ownership in priority data while keeping operations and CEO surfaces free of assignment noise')
...
console.log('Minimum-team flow contract PASS: reception, production, packing, inventory, commercial commitments, functional ownership and executive focus preserve capture-once, inherited context and exception-only review')
