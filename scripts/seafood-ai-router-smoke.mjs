...
assert(router.includes("SEAFOOD_QUERY_ROUTER_VERSION='seafood.router.v5'"),'query router must be explicitly versioned')
assert(router.includes('/\\bpor que\\b/')&&router.includes('/\\bwhy\\b/')&&router.includes('/\\bcompar/')&&router.includes('/\\bcompare/'),'router must preserve explicit bilingual routing intent')
...
assert(loader.includes('selectedBase.length?buildCopilotContext(operator,plantId,selectedBase)'),'deterministic lot routing must be able to skip the broad base context')
...
assert(loader.includes("selected.includes('urchin_graph')&&hasLot?buildSeaUrchinCopilotEvidence"),'urchin specialist must load only when routed')
...
assert(policy.includes("SEAFOOD_AI_POLICY_VERSION='seafood.ai.evidence.v13'"),'Seafood AI policy must be versioned at v13')
assert(policy.includes('capabilities autorizadas por Seafood AI Router'),'policy must bind the model to router scope')
