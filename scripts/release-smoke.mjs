...
assert(appSource.includes('<Route path="/" element={<DailyClose/>}/>'),'home route must remain minimal and task-first')
assert(appSource.includes('const DailyCloseAdvanced=lazy(()=>import("./pages/DailyCloseAdvanced")'),'advanced daily close must remain code-split outside the home bundle')
assert(appSource.includes('<Route path="/inicio/detalle" element={<><DailyCloseAdvanced/><ExecutiveDecisionBrief/><OperationalHealthPanel/><CanonicalIntelligencePanel/><CanonicalCategoryMix/><RollforwardReconciliation/><RollforwardResolutionQueue/></>}'),'reports must retain auditable daily close and detailed intelligence behind progressive disclosure')
assert(appSource.includes('<Route path="/cierre-diario" element={<Navigate to="/inicio/detalle" replace/>}/>'),'legacy daily-close route must remain reachable through reports')
