...
assert(appSource.includes('<Route path="/" element={<><DailyClose/><ExecutiveDecisionBrief/></>}'),'home route must remain minimal and decision-first')
assert(appSource.includes('<Route path="/inicio/detalle" element={<><OperationalHealthPanel/><CanonicalIntelligencePanel/><CanonicalCategoryMix/><RollforwardReconciliation/><RollforwardResolutionQueue/></>}'),'detailed home intelligence must remain available behind progressive disclosure')
