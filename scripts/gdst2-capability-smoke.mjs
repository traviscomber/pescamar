...
assert(foundation.includes("id:'master-data-resolution',state:'missing'"),'master data resolution must remain missing until implemented')
...
for(const registry of [serverRegistry,clientRegistry]){
  ...
  assert(adapter.includes("status:'foundation'"),'GDST adapter must be foundation once mapping contract exists')
...
console.log('GDST 2.0 capability smoke PASS: Event Graph mappings are explicit, missing EPCIS/Digital Link/master-data capabilities remain visible, writes are disabled and no GDST Capable claim is allowed')
