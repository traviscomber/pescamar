...
assert(policy.includes("SEAFOOD_AI_POLICY_VERSION='seafood.ai.evidence.v13'"),'Seafood AI policy must be explicitly versioned at v13')
...
assert(policy.includes('Cálculo:')&&policy.includes('Inferencia:')&&policy.includes('Si falta evidencia, dilo explícitamente'),'Seafood AI must distinguish calculation, inference and missing evidence')
...
assert(policy.includes('Cada afirmación factual, cálculo o inferencia debe citar una etiqueta [source]'),'system policy must tell the model about runtime citation enforcement')
assert(policy.includes('router.writesAllowed=false es vinculante')&&policy.includes('Nunca afirmes que ejecutaste, aprobaste o modificaste datos'),'Seafood AI must remain read-only in its policy')
assert(policy.includes('La ausencia de registros no prueba que un evento no ocurrió')&&intelligence.includes('missing_guide_price')&&intelligence.includes('missing_received_kg')&&intelligence.includes('missing_process_date')&&intelligence.includes('missing_production_date'),'Seafood AI must treat canonical production completeness gaps as missing evidence')
assert(policy.includes('Importes financieros parciales no son margen, utilidad ni caja')&&intelligence.includes("LEDGER_MOVEMENT_RULE='event_date is not null and (inflow_clp is not null or outflow_clp is not null)'"),'Seafood AI must define the audited ledger movement boundary')
assert(intelligence.includes('reference_rows'),'Seafood AI must preserve and classify ledger reference rows')
assert(policy.includes('Nunca las presentes como stock, recepción o prioridad live'),'Seafood AI must never promote reference rows into financial movements')
assert(policy.includes('operational_intelligence es la capa determinística de prioridades P1/P2/P3 del Seafood Event Graph'),'Seafood AI must use Event Graph operational intelligence as its priority layer')
assert(policy.includes('No inventes registros')&&policy.includes('Nunca reemplaces lot_control u operational_intelligence por una inferencia ML'),'Seafood AI must preserve deterministic signal priority')
assert(operationalContext.includes("evidenceBasis=aiProvider?'ai_extraction':'persisted_evidence'"),'Seafood AI must identify persisted AI extraction provenance')
assert(operationalContext.includes('evidenceBasis:typeof event.metrics.evidenceBasis'),'Seafood AI must keep persisted evidence separate from physical confirmation')
assert(policy.includes('Si ML contradice evidencia live determinística, prevalece la evidencia live'),'Seafood AI must preserve the human-vs-AI provenance boundary')
assert(policy.includes('router.writesAllowed=false es vinculante'),'Seafood AI must preserve the read-only operational intelligence boundary')
...
assert(page.includes('title="Seafood AI"')&&page.includes('no modifica registros'),'Pescamar IA must explain its evidence-native behavior in plain language')
assert(page.includes("'Lote + operación':'Lot + operation'"),'lot-scoped UI must describe the decision stack without internal jargon')
assert(page.includes('capabilityLabels')&&page.includes('Falta para afirmar más'),'missing capability IDs must be translated before being shown to operators')
...
assert(page.includes('Sólo lectura')&&page.includes('no modifica registros ni ejecuta decisiones'),'UI must keep the read-only trust boundary visible in plain language')
