...
const [registry,page,app,access,os,shell,lineage]=await Promise.all([
...
assert(registry.includes("id:'pescamar-univision-color'"),'Pescamar Uni Vision must be registered as the first adapter')
...
assert(registry.includes("status:'available_in_pescamar'")&&registry.includes("status:'planned'"),'capability registry must separate available evidence from planned work')
...
console.log('EdgeVision foundation smoke PASS: capability registry, human authority, route/access, OS navigation and Vision→Event Graph provenance verified')
