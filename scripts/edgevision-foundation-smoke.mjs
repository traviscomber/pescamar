...
assert(segmentApi.includes("requireOperator(request,['admin','operations','quality','finance','viewer'])"),'segmentation API must preserve the authenticated all-role /uni access contract')
...
assert(qaBench.includes('setCameraBusy(true);setCameraCycleOk(false)')&&qaBench.includes('finally{setCameraBusy(false);if(inputRef.current)'),'QA upload analysis must serialize remote requests and release busy state')
