...
assert(profitability.includes('from receptions r where ${admin} or r.plant_id=any(${plantIds}::text[])'),'live profitability must derive supplier and plant metrics from authorized receptions')
...
assert(profitability.includes("profitabilityRule:'unknown_is_not_zero'"),'profitability boundary must keep missing economic evidence unknown rather than zero')
assert(profitability.includes('bool_and(full_sale and purchase_known and transformation_known) economic_complete'),'profitability must require complete sold-lot economic lineage before contribution')
...
console.log('Financial read boundary smoke PASS: Quality cannot read financial/commercial APIs, daily-close amounts, or plant financial performance; settlement credit identity is exact-party only; profitability is plant-scoped and unknown costs fail closed; regulatory dispatch gates remain intact')
