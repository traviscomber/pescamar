...
test('Control Tower puts attributable live Event Graph priority first',async({page})=>{
 ...
 await expect(brief.getByRole('heading',{name:'Qué requiere atención',exact:true})).toBeVisible({timeout:15000})
...
test('Control Tower shows an honest zero-signal state',async({page})=>{
 ...
 await page.goto('/inicio/detalle')
 const brief=page.getByRole('region',{name:'Qué requiere atención'})
 await expect(brief.getByText('Sin pendientes en 3 lotes',{exact:true})).toBeVisible({timeout:15000})
