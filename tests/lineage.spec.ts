...
async function mockLineage(page:Page){
  await page.route('**/api/**',async route=>{
    ...
    ],coverage:{reception:true,evidence:false,quality:true,production:true,inventory:true,commercialCommitment:true,dispatch:false,sale:false},permissions:{canSeeCommercial:true}})})
...
test('Seafood Event Graph shows attributed lineage without inventing missing stages',async({page},testInfo)=>{
  ...
  await expect(page.locator('.lineage-events li')).toHaveCount(5)
