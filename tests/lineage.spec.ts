...
test('Trazabilidad actual shows attributed lineage without inventing missing stages',async({page},testInfo)=>{
  ...
  await expect(main.getByRole('heading',{name:'Trazabilidad actual'})).toBeVisible()
  await expect(main.getByText('Powered by Seafood Event Graph.',{exact:false})).toBeVisible()
  await expect(main.getByText('Pescamar',{exact:true}).first()).toBeVisible()
  await expect(main.getByText('6/9',{exact:true})).toBeVisible()
  await expect(main.getByText('Sin evento')).toHaveCount(3)
  await expect(main.getByRole('heading',{name:'Eventos atribuibles'})).toBeVisible()
  await expect(main.locator('.lineage-events li')).toHaveCount(6)
  await expect(main.getByText('Grade A · ΔE 1.42 · accepted · Station QA',{exact:true})).toBeVisible()
  await expect(main.getByText('Cliente QA',{exact:false}).first()).toBeVisible()
...
test('Trazabilidad histórica exposes canonical evidence without presenting it as live',async({page},testInfo)=>{
  ...
  await expect(page.getByRole('heading',{name:'Trazabilidad por período'})).toBeVisible()
  await expect(page.getByRole('combobox',{name:'Vista'})).toHaveValue('historical')
  await expect(page.getByText(/Reporte del período · solo lectura/)).toBeVisible()
  await expect(page.getByText('No modifica ni completa la operación actual.')).toBeVisible()
  await expect(page.getByRole('combobox',{name:'Registro'})).toContainText('mdn149220526')
  await expect(page.getByText('PERÍODO · SOLO LECTURA',{exact:true})).toBeVisible()
  ...
  await expect(page.locator('.lineage-events li')).toHaveCount(4)
