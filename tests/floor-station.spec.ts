...
test('Floor Station is scoped to operator plants and remains read-only',async({page})=>{
 ...
 await expect(page.getByText('Proveedor Ancud',{exact:false})).toBeVisible()
 await expect(page.getByText('Proveedor Quellón')).toHaveCount(0)
 await expect(page.getByText('Sin escrituras DB')).toBeVisible()
 const confirm=page.getByRole('button',{name:/Preparar packing unit/})
