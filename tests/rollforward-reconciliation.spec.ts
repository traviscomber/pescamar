...
test('roll-forward workspace links support evidence and leaves contradictions for human review',async({page},testInfo)=>{
 ...
 const evidence=page.locator('details').filter({hasText:'Isla Guafo · 3343'})
 await evidence.locator('summary').click()
 await expect(evidence.getByRole('columnheader',{name:'Kg aceptados'})).toBeVisible()
 await expect(evidence.getByRole('columnheader',{name:'D/RGA kg'})).toBeVisible()
