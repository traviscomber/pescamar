...
test('roll-forward workspace links support evidence and leaves contradictions for human review',async({page},testInfo)=>{
 ...
 await expect(conflict.getByText('8',{exact:true})).toBeVisible()
 await expect(conflict.getByText('Conflicto',{exact:true})).toBeVisible()
 await page.getByText('Isla Guafo · 3343',{exact:true}).click()
 await expect(page.getByRole('columnheader',{name:'Kg aceptados'})).toBeVisible()
 await expect(page.getByRole('columnheader',{name:'D/RGA kg'})).toBeVisible()
