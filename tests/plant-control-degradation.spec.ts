...
async function mock(page:Page){
 await page.route('**/api/**',async route=>{
  ...
  if(path==='/api/auth')return json({ok:true,operator:{id:'qa-admin',fullName:'QA Admin',email:'admin@example.test',role:'admin',plantIds:['ancud','quellon','iquique','piedra-azul','aqua-austral','natales']}})
  ...
  if(path==='/api/plant-state')return json({ok:true,plants:[sharedPlant],history:[],updatedAt:'2026-09-02T00:00:00.000Z'})
