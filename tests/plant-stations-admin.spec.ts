...
test('station administration reuses canonical Plant Execution API and stays admin-only',async()=>{
 ...
 expect(page).toContain("!data?.writesEnabled")
...
async function mock(page:Page,role:'admin'|'operations',writesEnabled=true){
 await page.route('**/api/**',async route=>{
  ...
  if(path==='/api/plant-stations')return json({ok:true,writesEnabled,stations:[]})
  return json({})
 })
}
...
test('read-only gate disables station and device mutations',async({page})=>{
 ...
 await expect(page.getByRole('button',{name:'Guardar dispositivo'})).toBeDisabled()
})
