...
test('supplier score keeps high score provisional when canonical mass evidence is weak',async({page},testInfo)=>{
 ...
 await expect(page.getByText('supplier-score-v1.2-canonical-evidence-gate')).toBeVisible()
 await expect(page.getByText('Filas revisión masa')).toBeVisible()
 await expect(page.getByText('Proveedor QA')).toBeVisible()
 await expect(page.getByText('Provisional · confianza baja · cobertura 65%')).toBeVisible()
 await page.getByText('Proveedor QA').click()
 await expect(page.getByText('4 filas revisión masa')).toBeVisible()
