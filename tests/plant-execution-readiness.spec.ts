...
test('rollout shows missing physical evidence as direct operational actions without promoting a plant to LIVE',async({page},testInfo)=>{
 ...
 await expect(page.getByRole('link',{name:'Ir a piso: Packing real'})).toHaveAttribute('href','/es/floor')
 await expect(page.getByRole('link',{name:'Preparar impresión: Etiqueta física confirmada'})).toHaveAttribute('href','/es/impresion-etiquetas')
 await expect(page.getByRole('link',{name:'Operar pallets: Pallet trazable'})).toHaveAttribute('href','/es/pallets')
 await expect(page.getByRole('link',{name:'Operar frío: Frío con evidencia'})).toHaveAttribute('href','/es/frio')
 await expect(page.getByRole('link',{name:'Ejercitar hold: Control regulatorio ejercitado'})).toHaveAttribute('href','/es/control-regulatorio')
