...
test('historical lot reconciliation is deterministic and evidence only',async()=>{
 ...
 expect(source).toContain("not in ('sin guia','sin guía','s/guia','s/guía')")
 expect(source).toContain("if(guides.length!==1)")
 expect(source).toContain("if(guidePlaceholderRows>0)reasons.push('guide_placeholder_present')")
