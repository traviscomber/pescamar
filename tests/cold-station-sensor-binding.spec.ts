...
test('cold page renders physical station and its sensor without overflow',async({page},testInfo)=>{
 ...
 await expect(page.getByLabel('Estación de frío')).toContainText('cold-01 · Frío 1')
 await page.getByLabel('Ciclo abierto').selectOption('44444444-4444-4444-8444-444444444444')
 await page.getByLabel('Origen lectura').selectOption('sensor')
 await expect(page.getByLabel('Sensor')).toContainText('SENSOR-QA-1')
...
test('cold controls respect the shared write gate',async({page})=>{
 ...
 await page.goto('/frio')
 await page.getByLabel('Estación de frío').selectOption('11111111-1111-4111-8111-111111111111')
 await page.getByLabel('Código').fill('TUN-QA')
 await page.getByLabel('Nombre').fill('Túnel QA')
