...
test('canonical intelligence renders uploaded-source KPIs without inventing yield',async({page},testInfo)=>{
 ...
 await expect(page.getByText('Reconciliar 100 filas de producción',{exact:true})).toBeVisible()
 ...
 await expect(page.getByText('19.384,2 kg',{exact:true})).toBeVisible()
 ...
 await expect(page.getByText('100 filas requieren reconciliar salidas reportadas',{exact:true})).toBeVisible()
