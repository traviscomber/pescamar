...
test('shell presents Seafood Intelligence OS as product and Pescamar as implementation',async({page},testInfo)=>{
  ...
  await expect(navigation.getByText('Pescamar',{exact:true})).toBeVisible()
  await expect(navigation.getByText('Implementation 01 · Instancia operativa',{exact:true})).toBeVisible()
