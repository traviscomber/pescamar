...
for(const surface of [
 ...
 test(`${surface.path} renders as an authenticated Plant Execution control on desktop and mobile`,async({page},testInfo)=>{
  ...
  await expect(page.getByRole('link',{name:'Piso / packing'})).toBeVisible()
  await expect(page.getByRole('link',{name:'Pallets'}).first()).toBeVisible()
  await expect(page.getByRole('link',{name:'Cadena de frío'}).first()).toBeVisible()
  await expect(page.getByRole('link',{name:'Regulatorio'})).toBeVisible()
