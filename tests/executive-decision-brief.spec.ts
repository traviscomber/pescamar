...
test('home surfaces one coherent purchase, evidence, supplier and plant decision brief',async({page},testInfo)=>{
 ...
 await expect(brief.getByText('88/89 vinculados',{exact:true})).toBeVisible()
 await expect(brief.getByText('1 por revisar',{exact:true})).toBeVisible()
 await expect(brief.getByText('Proveedor B',{exact:true})).toBeVisible()
 await expect(brief.getByText('quellon · 58%',{exact:true})).toBeVisible()
 await expect(brief.getByText(/Diaz termiando · bloque 99 · guía 180 · lote mdq106/)).toBeVisible()
