...
async function mockCopilot(page:Page){
 await page.route('**/api/**',async route=>{
  ...
  if(path==='/api/copilot')return route.fulfill({status:200,contentType:'application/json',body:JSON.stringify({ok:true,answer:'Hay 2 lotes con revisión de calidad [quality]. La evidencia histórica disponible registra packing canónico [canonical_inventory]. Inferencia: conviene resolver la revisión antes de comprometer disponibilidad.',engine:'Seafood AI',policyVersion:'seafood.ai.evidence.v8',generatedAt:'2026-09-03T12:00:00.000Z',scope:{plantId:'ancud',plantIds:['ancud'],role:'operations',organizationId:'pescamar'},sources:[{id:'quality',label:'Calidad y holds',path:'/control-regulatorio',rows:2,freshness:'2026-09-03T11:00:00.000Z',evidenceClass:'live_observation'},{id:'canonical_inventory',label:'Evidencia canónica de inventario',path:'/inventario',rows:562,freshness:'2026-08-31T00:00:00.000Z',evidenceClass:'canonical_history'}]})})
...
test('Seafood AI exposes evidence policy with scoped navigable evidence without overflow',async({page},testInfo)=>{
 ...
 await expect(page.getByRole('link',{name:'[quality]'})).toHaveAttribute('href','/es/control-regulatorio')
