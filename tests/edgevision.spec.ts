...
test('EdgeVision separates available software evidence from planned capabilities',async({page},testInfo)=>{
 ...
 await expect(page.getByRole('heading',{name:'EdgeVision',exact:true})).toBeVisible()
 await expect(page.getByText('2',{exact:true}).first()).toBeVisible()
 await expect(page.getByText('Pescamar · Uni Vision',{exact:true})).toBeVisible()
 await expect(page.getByText(/revisión humana obligatoria/i)).toBeVisible()
 await expect(page.getByText(/versionado reusable de modelo\/engine: pendiente/i)).toBeVisible()
 await expect(page.getByText('Sin modelo operacional conectado todavía.')).toHaveCount(3)
 await expect(page.getByRole('link',{name:/Seafood Event Graph/})).toBeVisible()
 await expect(page.getByRole('link',{name:/Estaciones/})).toBeVisible()
...
test('EdgeVision does not expose admin station configuration to viewers',async({page})=>{
 ...
 await expect(page.getByRole('heading',{name:'EdgeVision',exact:true})).toBeVisible()
 await expect(page.getByRole('link',{name:/Seafood Event Graph/})).toBeVisible()
 await expect(page.getByRole('link',{name:'Estaciones',exact:true})).toHaveCount(0)
})
